...
const HARD_REJECT_KEYWORDS = [
  ...
  "부품용", "파손", "고장", "침수", "수리이력", "액정파손",
  "매입", "삽니다", "구해요", "구매합니다", "구합니다",
  "a7c2", "a7cii", "a7c ii", "a7cm2", "7cm2", "ilce 7cm2", "ilce-7cm2",
  "a7m3", "a7m4", "a7r", "a7s", "fx3", "fx30",
  "canon", "캐논", "nikon", "니콘", "fujifilm", "후지",
  "렌즈킷", "렌즈 킷", "lens kit", "kit lens",
  "28-60", "28 60", "sel28-60",
  "탐론", "tamron", "시그마", "sigma",
];
...
async function postRows(table: string, rows: Record<string, unknown>[], onConflict: string) {
  ...
  if (!res.ok) { const text = await res.text(); throw new Error(`${table} upsert failed: ${res.status} ${text.slice(0,300)}`); }
}

async function existingPids(): Promise<Set<number>> {
  const res = await restFetch(`${tableUrl("mvp_raw_listings")}?select=pid&sku_id=eq.${TARGET.skuId}`, { headers: serviceHeaders() });
...
async function main() {
  ...
  console.log(`existing Sony A7C pids: ${skipPids.size}`);
  ...
  for (const item of items.values()) {
    ...
    const hasModel = norm.includes("a7c") || norm.includes("ilce7c") || norm.includes("ilce-7c");
  ...
  for (const cand of candidates) {
    ...
    if (!parsed.comparableKey || !parsed.comparableKey.startsWith("camera|")) { cand.rejectReason = `comparable_key_unexpected:${parsed.comparableKey ?? "null"}`; continue; }
    cand.fresh = fresh; cand.comparableKey = parsed.comparableKey;
    validated.push(cand);
  }
  ...
  for (const cand of validated) {
    ...
    rawRows.push({
      ...
      query: `wave18_camera_sony_a7c_boost:${TARGET.skuId}`, source: "bunjang",
      description_preview: fresh.description ?? "", sale_status: fresh.saleStatus ?? "",
      seller_source: "bunjang", shop_review_rating: fresh.shopReviewRating ?? null,
      shop_review_count: fresh.shopReviewCount ?? 0, seller_uid: fresh.shopUid ?? null, seller_name: fresh.shopName ?? null,
      trade_data: fresh.tradeData ?? null, trades_data: fresh.tradesData ?? null,
      image_url_template: fresh.imageUrlTemplate ?? null, image_count: fresh.imageCount ?? 0, thumbnail_url: fresh.thumbnailUrl ?? null,
      listing_type: "normal", sku_id: matched.id, sku_name: matched.modelName,
      detail_status: "done", detail_enriched_at: now, listing_state: "active", pool_eligible: true, score_dirty: true,
      last_seen_at: now, last_changed_at: now, updated_at: now,
      raw_json: { source: "wave18_camera_sony_a7c_boost", lane: TARGET.skuId },
    });
  ...
  for (const c of candidates) { if (!c.rejectReason) continue; const key = c.rejectReason.split(":")[0]; histogram[key] = (histogram[key] ?? 0) + 1; }
  const report = {
    generatedAt: new Date().toISOString(), startedAt, reportOnly: !applyMode,
    category: "wave18_camera_sony_a7c_boost",
    metrics: { searchCollected: items.size, preFilterPass: candidates.length, validated: validated.length, applied: applyMode ? rawRows.length : 0 },
    rejectionHistogram: histogram, expectedComparableKeys: [...expectedKeys],
    validatedPids: validated.map((c) => ({ pid: c.pid, price: c.price, comparableKey: c.comparableKey, title: c.title.slice(0, 80) })),
  };
  await mkdir(reportsDir, { recursive: true });
  await writeFile(path.join(reportsDir, "wave18-camera-sony-a7c-boost-latest.json"), JSON.stringify(report, null, 2));
  console.log(`wave18 sony a7c: collected=${items.size}, validated=${validated.length}, applied=${applyMode ? rawRows.length : 0}, expectedKeys=${[...expectedKeys].join(",")}`);
}
