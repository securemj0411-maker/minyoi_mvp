...
type Finding = { severity: "hard" | "soft"; file: string; rule: string; detail: string };

async function main(): Promise<void> {
  const registry = JSON.parse(
    await readFile(path.join(reportsDir, "phones-packet-registry-latest.json"), "utf8"),
  ) as Registry;
  ...
  for (const entry of registry.entries) {
    if (entry.reportOnly !== true)
      findings.push({
        ...
        detail: `reportOnly=${entry.reportOnly}.`,
      });
    ...
    try {
      ...
      findings.push({ severity: "hard", file: entry.file, rule: "invalid_json", detail: "cannot parse json." });
      continue;
    }
  ...
  const report = {
    ...
    decision: "phones_packet_audit_report_only",
    totals: { packets: registry.entries.length, hardFindings: hardCount, softFindings: softCount, pass: hardCount === 0 },
    findings,
  };

  await mkdir(reportsDir, { recursive: true });
  const jsonPath = path.join(reportsDir, "phones-packet-audit-latest.json");
  ...
  const md = [
    ...
    "",
    "Report-only audit checking report-only invariants across all phones packets.",
    "",
    "## Totals",
    "",
  ...
  console.log(
    `phones-packet-audit: packets=${report.totals.packets}, hard=${hardCount}, soft=${softCount}, pass=${report.totals.pass}`,
  );
}
