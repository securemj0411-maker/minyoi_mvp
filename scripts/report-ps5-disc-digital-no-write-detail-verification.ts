...
const sourcePath = path.join(reportDir, "exact-acquisition-no-write-sample-ps5_disc_digital_standard_wave1-latest.json");
...
function ps5DetailReasons(text: string, skuId: string | null) {
  const normalized = norm(text);
  const reasons: string[] = [];
  if (!/(ps5|플스\s*5|플스5|플레이스테이션\s*5|playstation\s*5)/i.test(normalized)) {
    reasons.push("missing_ps5_detail_context");
  }
  if (!/(디스크|disc|디지털|digital|cd|씨디|cd롬)/i.test(normalized)) {
    reasons.push("missing_disc_or_digital_detail_context");
  }
  if (/(슬림|slim|프로|pro|ps5pro|ps5\s*pro|플스5\s*프로|vr|psvr|포탈|portal|스위치|switch)/i.test(normalized)) {
    reasons.push("ps5_wrong_model_signal");
  }
  if (/(패드\s*만|컨트롤러\s*만|듀얼센스\s*만|충전\s*거치대|충전거치대|케이스|커버|스킨|부품|디스크\s*드라이브\s*만)/i.test(normalized)) {
    reasons.push("ps5_accessory_only_signal");
  }
  if (/(게임\s*만|타이틀\s*만|cd\s*만|소프트웨어|계정|다운로드|dl\s*코드)/i.test(normalized)) {
    reasons.push("ps5_game_or_account_signal");
  }
  if (/(게임|타이틀|페르소나|스파이더맨|마일즈|듀얼센스\s*2개|컨트롤러\s*2개|패드\s*2개|\+\s*듀얼센스|\+\s*게임)/i.test(normalized)) {
    reasons.push("ps5_bundle_price_review");
  }
  if (/(삽니다|매입|구합니다|구매합니다)/i.test(normalized)) {
    reasons.push("buying_signal");
  }
  if (/(고장|파손|수리|침수|전원\s*안|부팅\s*안|불량|부품용)/i.test(normalized)) {
    reasons.push("damaged_or_parts_signal");
  }
  if (skuId && !["ps5-disc-standard", "ps5-digital-standard"].includes(skuId)) {
    reasons.push("non_standard_ps5_sku");
  }
...
async function main() {
  const limit = intArg("limit", 12, 1, 30);
  const source = JSON.parse(await readFile(sourcePath, "utf8")) as SampleReport;
  const inputRows = (source.rows ?? [])
    .filter((row) => row.decision !== "hold" && ["ps5-disc-standard", "ps5-digital-standard"].includes(row.skuId ?? ""))
    .slice(0, limit);
  const rows = [];
  ...
  for (const row of inputRows) {
    ...
    const reasons = [
      ...
      ...ps5DetailReasons(text, sku?.id ?? row.skuId),
    ];
    const activeClean =
      Boolean(detail) &&
      !sold &&
  ...
  const output = {
    ...
    source: "reports/exact-acquisition-no-write-sample-ps5_disc_digital_standard_wave1-latest.json",
    scope: "ps5_disc_digital_standard_no_write_detail_verification",
    ...
    decision:
      activeCleanRows.length >= 4
        ? "PS5 Disc/Digital standard has enough no-write active clean detail rows for an owner-reviewed tiny acquisition design. Bundle rows remain review-only."
        : "PS5 Disc/Digital standard needs more exact no-write detail evidence or a bundle normalization policy before any acquisition design.",
  };

  const md = [
    "# PS5 Disc/Digital No-Write Detail Verification",
    "",
  ...
  await writeFile(path.join(reportDir, "ps5-disc-digital-no-write-detail-verification-latest.json"), `${JSON.stringify(output, null, 2)}\n`);
  await writeFile(path.join(reportDir, "ps5-disc-digital-no-write-detail-verification-latest.md"), md);
  console.log("wrote reports/ps5-disc-digital-no-write-detail-verification-latest.json");
  console.log("wrote reports/ps5-disc-digital-no-write-detail-verification-latest.md");
  console.log(
    JSON.stringify({
      inputRows: output.inputRows,
      activeClean: output.activeClean,
