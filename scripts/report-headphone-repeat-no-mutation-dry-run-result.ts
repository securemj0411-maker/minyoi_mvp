...
function decide(row: FixtureRow): { decision: DryRunDecision; reason: string; failureIf: string } {
  if (row.expectedClass === "manual_review") {
    return {
      ...
      reason: "Manual-review fixture must not be promoted by title token inference, even when connector or full-box wording is present.",
      failureIf: "This row is emitted as candidate_positive_only without owner-approved generation/connector policy.",
    };
  }

  if (
    row.accessoryOrPartTokens.length > 0 ||
    hasAnyToken(row, ["쿠션", "케이스", "박스", "case", "accessory", "bundle"])
  ) {
    return {
      decision: "negative_hold_only",
      reason: "Accessory, case, part, or bundle-like tokens are present, so this stays out of positive comparable candidates.",
      failureIf: "This row is emitted as candidate_positive_only or any runtime/public flag becomes true.",
    };
  }

  if (hasAnyToken(row, ["레이저", "비츠", "beats", "blackshark"]) && row.expectedClass === "hold") {
    return {
      decision: "negative_hold_only",
      reason: "Unknown branded headphone/SKU is outside the matched-SKU subset and needs separate catalog expansion.",
      failureIf: "This row is emitted as candidate_positive_only without separate brand/SKU catalog evidence.",
    };
  }
...
async function main(): Promise<void> {
  ...
  const fixture = await readJson<FixturePacket>(path.join(reportsDir, "headphone-repeat-dry-run-expanded-fixture-packet-latest.json"));
  ...
  const report = {
    ...
    category: "headphone_discovered",
    scope: "Repeat headphone no-mutation parser dry-run result; expanded fixture/report-only inputs only",
    inputFiles: [
    ...
    outputFiles: [
      ...
      "reports/headphone-repeat-no-mutation-dry-run-result-latest.md",
    ],
    metrics: {
    ...
    decision: validationErrors.length === 0 && missingRequiredFields.length === 0 && resultRows.every((row) => row.pass)
      ? "repeat_dry_run_pass_report_only_no_runtime_approval"
      : "repeat_dry_run_failed_report_only",
  };

  await mkdir(reportsDir, { recursive: true });
  await writeFile(path.join(reportsDir, "headphone-repeat-no-mutation-dry-run-result-latest.json"), JSON.stringify(report, null, 2));
  ...
  const md = [
    "# Headphone Repeat No-Mutation Dry-Run Result",
    "",
    `Generated: ${generatedAt}`,
    "",
    "Report-only repeat dry-run result for headphone_discovered. This did not import runtime parser modules, mutate runtime files, promote public readiness, wire candidate pools, or write to production DB.",
    "",
    "## Metrics",
    "",
  ...
  await writeFile(path.join(reportsDir, "headphone-repeat-no-mutation-dry-run-result-latest.md"), `${md}\n`);
  console.log("wrote reports/headphone-repeat-no-mutation-dry-run-result-latest.json");
  console.log("wrote reports/headphone-repeat-no-mutation-dry-run-result-latest.md");
  console.log(`headphone repeat no-mutation dry-run: rows=${report.metrics.rows}, passed=${report.metrics.passedRows}, failed=${report.metrics.failedRows}, validation_errors=${report.metrics.validationErrors}`);
}
