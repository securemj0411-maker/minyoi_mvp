...
async function main() {
  ...
  console.log("[1/3] fetching ipad-pro broad matter with 12.9 notation (30d)...");
  const since = new Date(Date.now() - 30 * 24 * 3600 * 1000).toISOString();
  const r = await fetch(
    `${URL_BASE}/mvp_raw_listings?select=pid,name,description_preview,sku_id` +
    `&sku_id=eq.ipad-pro&listing_state=eq.active&first_seen_at=gte.${since}` +
    `&or=(name.ilike.*12.9*,description_preview.ilike.*12.9*)` +
    `&limit=1000`,
    { headers: HDR },
  );
