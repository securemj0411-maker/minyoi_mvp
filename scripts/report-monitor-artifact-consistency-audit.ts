...
const files = [
  ...
  "reports/monitor-pending-model-spec-evidence-latest.md",
...
const report = {
  ...
  scope: "monitor report-only artifact consistency audit",
  metrics: {
  ...
  conclusion:
    failures.length === 0
      ? "monitor_artifact_consistency_passed_report_only"
      : "monitor_artifact_consistency_failed_review_required",
  nextAction: "Continue to desktop full-unit versus part-only fixture expansion packet.",
};

const reportsDir = path.join(process.cwd(), "reports");
const jsonPath = path.join(reportsDir, "monitor-artifact-consistency-audit-latest.json");
const mdPath = path.join(reportsDir, "monitor-artifact-consistency-audit-latest.md");

fs.writeFileSync(jsonPath, `${JSON.stringify(report, null, 2)}\n`);

const md = [
  "# Monitor Artifact Consistency Audit",
  "",
