...
const HARD_REJECT_KEYWORDS = [
  "어댑터만", "케이블만", "스탠드만", "박스만", "케이스만", "거치대만",
  "부품용", "파손", "고장", "수리이력", "액정파손",
  "매입", "삽니다", "구해요", "구매합니다", "구합니다",
  // 다른 LG 4K 모니터 변형 격리
  "27ul850", "27uk850", "27up550", "27up600", "27us550",
  // gaming UltraGear와 격리
  "27gp850", "27gn850", "ultragear", "울트라기어",
  // 32"/24" 다른 사이즈
  "32up850", "32up95", "24up850",
  // 타 브랜드
  "삼성", "samsung", "오디세이", "odyssey", "dell", "alienware", "에이수스", "asus", "벤큐", "benq",
];
...
async function main() {
  ...
  console.log(`existing LG 27UP850N pids: ${skipPids.size}`);
  ...
  for (const item of items.values()) {
    ...
    // require 27up850 model code in title
    if (!norm.includes("27up850")) continue;
  ...
  for (const cand of candidates) {
    ...
    if (!parsed.comparableKey || !parsed.comparableKey.startsWith("monitor|")) {
  ...
  for (const cand of validated) {
    ...
    rawRows.push({
      ...
      query: `wave24_monitor_lg_27up850n_boost:${TARGET.skuId}`, source: "bunjang",
      ...
      raw_json: { source: "wave24_monitor_lg_27up850n_boost", lane: TARGET.skuId },
    });
    parsedRows.push(toParsedListingRow(cand.pid, parsed));
  }

  if (applyMode && rawRows.length > 0) {
    if (process.env.WAVE24_WRITE_APPROVED !== "1") throw new Error("Apply refused: set WAVE24_WRITE_APPROVED=1");
  ...
  const report = {
    ...
    category: "wave24_monitor_lg_27up850n_boost",
    metrics: {
  ...
  const jsonPath = path.join(reportsDir, "wave21-monitor-27up850n-boost-latest.json");
  await writeFile(jsonPath, JSON.stringify(report, null, 2));
  console.log(`wave24 lg 27up850n: collected=${items.size}, validated=${validated.length}, applied=${applyMode ? rawRows.length : 0}, expectedKeys=${[...expectedKeys].join(",")}`);
}
