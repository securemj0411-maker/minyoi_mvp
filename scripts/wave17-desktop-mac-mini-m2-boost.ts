...
const HARD_REJECT_KEYWORDS = [
  "풀세트", "풀구성",
  "케이스만", "박스만", "어댑터만", "케이블만", "스탠드만",
  "부품용", "부품 용", "파손", "고장", "침수", "수리이력",
  "매입", "삽니다", "구해요", "구매합니다", "구합니다",
  "리퍼폰", "리퍼 폰",
  // Mac mini 다른 chip / variant 제외
  "m1", "m3", "m4", "m2 pro", "m2pro", "m2 ultra", "m2ultra",
  "mac studio", "맥스튜디오", "imac", "아이맥",
  "macbook", "맥북",
...
async function main() {
  ...
  console.log(`existing Mac mini M2 256 pids: ${skipPids.size}`);
  ...
  for (const item of items.values()) {
    ...
    // require "macmini" + "m2"
    if (!((norm.includes("macmini") || norm.includes("맥미니")) && norm.includes("m2"))) continue;
  ...
  for (const cand of validated) {
    ...
    rawRows.push({
      ...
      query: `wave17_desktop_mac_mini_boost:${TARGET.skuId}`, source: "bunjang",
      ...
      raw_json: { source: "wave17_desktop_mac_mini_boost", lane: TARGET.skuId },
    });
    parsedRows.push(toParsedListingRow(cand.pid, parsed));
  }

  if (applyMode && rawRows.length > 0) {
    if (process.env.WAVE17_WRITE_APPROVED !== "1") throw new Error("Apply refused: set WAVE17_WRITE_APPROVED=1");
  ...
  const report = {
    ...
    category: "wave17_desktop_mac_mini_m2_boost",
    metrics: {
  ...
  const jsonPath = path.join(reportsDir, "wave17-desktop-mac-mini-m2-boost-latest.json");
  await writeFile(jsonPath, JSON.stringify(report, null, 2));
  console.log(`wave17 desktop mac mini m2: collected=${items.size}, validated=${validated.length}, applied=${applyMode ? rawRows.length : 0}, expectedKeys=${[...expectedKeys].join(",")}`);
}
