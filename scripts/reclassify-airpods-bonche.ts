...
async function main() {
  ...
  console.log("[1/3] fetching '본체' 포함 매물 (sku_id null, 14d)...");
  ...
  const r = await fetch(
    ...
    `&or=(name.ilike.*에어팟*,name.ilike.*airpods*)` +
  ...
  console.log("[2/3] re-classifying via classifyListing (pipeline fix)...");
  const changes: { pid: number; to: string }[] = [];
  const broadIds: Record<string, number> = {};
  for (const row of rows) {
    const r = classifyListing(row.name ?? "", row.description_preview ?? "", row.price ?? 50000);
    if (r.listingType === "normal" && r.sku) {
      changes.push({ pid: row.pid, to: r.sku.id });
      broadIds[r.sku.id] = (broadIds[r.sku.id] ?? 0) + 1;
    }
  }
