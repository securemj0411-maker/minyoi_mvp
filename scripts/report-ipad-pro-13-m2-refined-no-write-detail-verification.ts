...
const sourcePath = path.join(reportDir, "exact-acquisition-no-write-sample-ipad_pro_13_m2_refined_wave2-latest.json");
...
function ipadDetailReasons(text: string) {
  const normalized = norm(text);
  const reasons: string[] = [];

  if (!/(아이패드|ipad)/i.test(normalized) || !/(프로|pro)/i.test(normalized)) {
    reasons.push("missing_ipad_pro_detail_context");
  }
  if (!/(12\.9|12\s*9|13\s*인치|13\s*형)/i.test(normalized)) {
    reasons.push("missing_12_9_detail_context");
  }
  if (!/(6세대|6\s*th|m2)/i.test(normalized)) {
    reasons.push("missing_m2_or_6th_detail_context");
  }
  if (!/256/.test(normalized)) {
    reasons.push("missing_256_detail_context");
  }
  if (!/(wifi|wi-fi|와이파이|와파|wlan)/i.test(normalized)) {
    reasons.push("missing_wifi_detail_context");
  }
  if (/(4세대|5세대|m1|a2378|a2379|a2461|a2462|아이패드\s*에어|ipad\s*air)/i.test(normalized)) {
    reasons.push("wrong_generation_or_air_signal");
  }
  if (/(셀룰러|cellular|\blte\b|\b5g\b|유심|esim|wi-?fi\s*\+\s*cell|\bcell\b)/i.test(normalized)) {
    reasons.push("cellular_conflict_signal");
  }
  if (/(매입|삽니다|구합니다|구매합니다)/i.test(normalized)) {
    reasons.push("buying_signal");
  }
  if (/(액정\s*만|부품|파손|고장|침수|잠김|락걸|icloud|아이클라우드|메인보드)/i.test(normalized)) {
    reasons.push("damaged_or_parts_signal");
  }
  if (/(케이스\s*만|펜슬\s*만|애펜\s*만|키보드\s*만|매직키보드\s*만|폴리오\s*만)/i.test(normalized)) {
    reasons.push("accessory_only_signal");
  }
  if (/(애플\s*펜슬|애펜|pencil|매직\s*키보드|magic\s*keyboard|키보드\s*포함|펜슬\s*포함|풀박스\s*\+\s*애펜|구성품\s*포함)/i.test(normalized)) {
    reasons.push("bundle_price_review");
  }
...
async function main() {
  const limit = intArg("limit", 10, 1, 30);
  ...
  const output = {
    ...
    source: "reports/exact-acquisition-no-write-sample-ipad_pro_13_m2_refined_wave2-latest.json",
    scope: "ipad_pro_13_m2_refined_no_write_detail_verification",
    ...
    decision:
      activeCleanRows.length >= 6
        ? "iPad Pro 13 M2 refined query has enough no-write active clean detail rows for a tiny owner-reviewed acquisition design. Bundle rows remain review-only."
        : "iPad Pro 13 M2 refined query still needs another no-write detail wave or tighter bundle policy before any acquisition design.",
  };

  const md = [
    "# iPad Pro 13 M2 Refined No-Write Detail Verification",
    "",
  ...
  await writeFile(path.join(reportDir, "ipad-pro-13-m2-refined-no-write-detail-verification-latest.json"), `${JSON.stringify(output, null, 2)}\n`);
  await writeFile(path.join(reportDir, "ipad-pro-13-m2-refined-no-write-detail-verification-latest.md"), md);
  console.log("wrote reports/ipad-pro-13-m2-refined-no-write-detail-verification-latest.json");
  console.log("wrote reports/ipad-pro-13-m2-refined-no-write-detail-verification-latest.md");
  console.log(
    JSON.stringify({
      inputRows: output.inputRows,
      activeClean: output.activeClean,
