...
async function main() {
  ...
  for (let iter = 1; iter <= 3; iter++) {
    ...
    const r = await fetch(
      ...
      `&or=(name.ilike.*갤럭시*,name.ilike.*z플립*,name.ilike.*z폴드*,name.ilike.*아이폰*,name.ilike.*ps5*,name.ilike.*맥미니*,name.ilike.*bose*,name.ilike.*보스*,name.ilike.*sony*,name.ilike.*소니*)` +
    ...
    if (changes.length === 0) { console.log("stop"); return; }
    const CHUNK = 50;
