...
async function main() {
  const samples = JSON.parse(await readFile("category-intelligence/ipad_mini_7_128_wifi/parse_ready_sample.json", "utf-8"));
  ...
  for (const row of rows as Array<{name?:string;description?:string}>) {
    ...
    if (r?.laneKey==="ipad_mini_7_128_wifi") narrow++;
    else if (r?.laneKey) { wrong++; wrongIds[r.id]=(wrongIds[r.id]??0)+1; }
    else if (r) { broad++; broadIds[r.id]=(broadIds[r.id]??0)+1; if(fails.length<5)fails.push({n:row.name??"",d:(row.description??"").slice(0,80),r:r.id}); }
    else { nullC++; if(fails.length<10)fails.push({n:row.name??"",d:(row.description??"").slice(0,80),r:"null"}); }
  }
  console.log(`Total: ${total}, target: ${narrow}(${(narrow/total*100).toFixed(1)}%), wrong: ${wrong}, broad: ${broad}, null: ${nullC}`);
