...
const files = [
  ...
  "reports/camera-false-merge-risk-matrix-latest.md",
  "reports/camera-package-split-prep-latest.md",
...
const report = {
  ...
  scope: "camera report-only artifact consistency audit",
  metrics: {
  ...
  conclusion:
    failures.length === 0
      ? "camera_artifact_consistency_passed_report_only"
      : "camera_artifact_consistency_failed_review_required",
  nextAction: "Continue to home-appliance vacuum subtype fixture expansion packet.",
};

const reportsDir = path.join(process.cwd(), "reports");
const jsonPath = path.join(reportsDir, "camera-artifact-consistency-audit-latest.json");
const mdPath = path.join(reportsDir, "camera-artifact-consistency-audit-latest.md");

fs.writeFileSync(jsonPath, `${JSON.stringify(report, null, 2)}\n`);

const md = [
  "# Camera Artifact Consistency Audit",
  "",
