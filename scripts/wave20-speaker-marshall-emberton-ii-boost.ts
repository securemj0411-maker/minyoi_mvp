...
const TARGET = {
  skuId: "speaker-marshall-emberton-ii",
  queries: [
    "마샬 엠버튼 2",
    "마샬 엠버튼 II",
...
const HARD_REJECT_KEYWORDS = [
  ...
  // Marshall 다른 portable / 거치형 라인업 분리
  "stockwell", "스탁웰", "kilburn", "킬번", "willen", "윌렌", "tufton", "터프턴", "middleton", "미들턴",
  "stanmore", "스탠모어", "acton", "액턴", "woburn", "워번",
  // 헤드폰/이어폰
  "major", "minor", "monitor ii", "마샬 헤드폰", "마샬 이어폰",
  // 기타 앰프 (브랜드 충돌)
  "기타 앰프", "guitar amp", "콤보앰프", "기타앰프",
  // 타 브랜드 portable
  "jbl", "bose", "soundlink", "boombox", "샤오미", "sonos", "소노스",
...
async function main() {
  ...
  console.log(`existing Marshall Emberton II pids: ${skipPids.size}`);
  ...
  for (const item of items.values()) {
    ...
    // require marshall + emberton II (마샬 + 엠버튼 2/II)
    const hasBrand = norm.includes("marshall") || norm.includes("마샬");
    const hasEmberton = norm.includes("emberton") || norm.includes("엠버튼");
    const hasGen2 = norm.includes("ii") || norm.includes("emberton2") || norm.includes("emberton 2") || norm.includes("엠버튼2") || norm.includes("엠버튼 2") || norm.includes("emberton ii") || norm.includes("엠버튼 ii");
    if (!(hasBrand && hasEmberton && hasGen2)) continue;
  ...
  for (const cand of validated) {
    ...
    rawRows.push({
      ...
      query: `wave20_speaker_marshall_emberton_ii_boost:${TARGET.skuId}`, source: "bunjang",
      ...
      raw_json: { source: "wave20_speaker_marshall_emberton_ii_boost", lane: TARGET.skuId },
    });
  ...
  const report = {
    ...
    category: "wave20_speaker_marshall_emberton_ii_boost",
    metrics: {
  ...
  const jsonPath = path.join(reportsDir, "wave20-speaker-marshall-emberton-ii-boost-latest.json");
  await writeFile(jsonPath, JSON.stringify(report, null, 2));
  console.log(`wave20 marshall emberton ii: collected=${items.size}, validated=${validated.length}, applied=${applyMode ? rawRows.length : 0}, expectedKeys=${[...expectedKeys].join(",")}`);
}
