...
const files = [
  "reports/desktop-full-unit-part-split-fixture-packet-latest.json",
  "reports/desktop-full-unit-part-split-fixture-packet-latest.md",
  "reports/desktop-full-unit-blockers-latest.md",
  "reports/desktop-cpu-gpu-implementation-prep-latest.md",
  "reports/desktop-no-mutation-runtime-dry-run-latest.json",
...
const report = {
  ...
  scope: "desktop report-only artifact consistency audit",
  metrics: {
  ...
  conclusion:
    failures.length === 0
      ? "desktop_artifact_consistency_passed_report_only"
      : "desktop_artifact_consistency_failed_review_required",
  nextAction: "Continue to speaker model-family and device-class fixture expansion packet.",
};

const reportsDir = path.join(process.cwd(), "reports");
const jsonPath = path.join(reportsDir, "desktop-artifact-consistency-audit-latest.json");
const mdPath = path.join(reportsDir, "desktop-artifact-consistency-audit-latest.md");

fs.writeFileSync(jsonPath, `${JSON.stringify(report, null, 2)}\n`);

const md = [
  "# Desktop Artifact Consistency Audit",
  "",
