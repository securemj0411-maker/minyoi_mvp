...
async function main() {
  ...
  console.log(`existing LG 27GP850 pids: ${skipPids.size}`);
  ...
  for (const item of items.values()) {
    ...
    // require 27gp850 model code in title
    if (!norm.includes("27gp850")) continue;
  ...
  for (const cand of validated) {
    ...
    rawRows.push({
      ...
      query: `wave24_monitor_lg_27gp850_boost:${TARGET.skuId}`, source: "bunjang",
      ...
      raw_json: { source: "wave24_monitor_lg_27gp850_boost", lane: TARGET.skuId },
    });
  ...
  const report = {
    ...
    category: "wave24_monitor_lg_27gp850_boost",
    metrics: {
  ...
  const jsonPath = path.join(reportsDir, "wave21-monitor-27gp850-boost-latest.json");
  await writeFile(jsonPath, JSON.stringify(report, null, 2));
  console.log(`wave24 lg 27gp850: collected=${items.size}, validated=${validated.length}, applied=${applyMode ? rawRows.length : 0}, expectedKeys=${[...expectedKeys].join(",")}`);
}
