...
const LANES: Record<LaneKey, LaneConfig> = {
  ...
  ipad_pro_11_m2_256_wifi: {
    ...
    queries: [
      ...
      "아이패드 프로 4세대 11 256",
      "ipad pro 4세대 11 256",
      "아이패드 프로 11 4세대 m2 256",
      "아이패드 프로 m2 256gb",
      "아이패드 프로 11 m2 256기가",
      "아이패드 프로 11 m2 wifi 256",
      "ipad pro m2 11 wifi 256",
      "아이패드 프로 11인치 m2 256",
      "ipad pro 11 inch m2 256",
    ],
    pages: 10,
    targetParseReady: 200,
    priceMin: 500_000,
    ...
    acceptAnyOf: [[/\bm2\b|m2\s*칩|\(m2\)/i, /4\s*세대|아이패드\s*프로\s*4세대|프로\s*11\s*4세대|11\s*프로\s*4세대|4th\s*gen/i]],
    reject: [],
    rejectLabelled: [
      { label: "wrong_chip_m1_m3_m4", pattern: /\bm[134]\b|m1\s*칩|m3\s*칩|m4\s*칩/i },
      { label: "wrong_storage_512_1tb_2tb", pattern: /512\s*(?:gb|기가)?|1\s*tb|2\s*tb|1\s*테라|2\s*테라/i },
      { label: "wrong_storage_128", pattern: /(?:^|[^0-9])128\s*(?:gb|기가)?\b/i },
      { label: "wrong_size_13_inch_or_12_9", pattern: /13\s*인치|13\s*형|12\.9\s*인치|13"|13″|아이패드\s*프로\s*6세대|6세대\s*프로/i },
      { label: "cellular_variant", pattern: /셀룰러|cellular|\blte\b|\b5g\b|유심|sim\s*트레이|esim/i },
      { label: "accessory_only", pattern: /케이스\s*(?:만|단품|개별)|필름\s*(?:만|단품)|키보드\s*만|펜슬\s*만|어댑터\s*만|충전기\s*만/i },
      { label: "case_or_smart_folio_listing", pattern: /(?:스마트\s*폴리오|스마트\s*커버|폴리오\s*케이스).{0,8}판매|매직\s*키보드\s*판매/i },
      { label: "broken_or_parts_only", pattern: /액정\s*파손|부품\s*용|부품용|파손\s*품/i },
  ...
  iphone_12_pro_128gb_self: {
    ...
    priceMin: 150_000,
    priceMax: 900_000,
    acceptAll: [/아이폰\s*12\s*(?:프로|pro)|iphone\s*12\s*(?:pro|프로)/i],
    acceptAnyOf: [],
    reject: [],
    rejectLabelled: [
      { label: "wrong_model_pro_max", pattern: /프로\s*맥스|promax|pro\s*max|프맥/i },
      { label: "wrong_model_12_mini_or_plus", pattern: /아이폰\s*12\s*미니|iphone\s*12\s*mini|아이폰\s*12\s*플러스|iphone\s*12\s*plus|아이폰\s*12\s*기본|\b12\s*mini\b|12\s*미니/i },
      { label: "wrong_model_11", pattern: /아이폰\s*11(?!\d)|iphone\s*11(?!\d)/i },
      { label: "wrong_model_13", pattern: /아이폰\s*13(?!\d)|iphone\s*13(?!\d)/i },
  ...
  iphone_13_pro_128gb_self: {
    ...
    priceMin: 350_000,
    priceMax: 1_200_000,
    acceptAll: [/(?:아이폰|iphone)\s*13\s*(?:프로|pro)/i],
    acceptAnyOf: [],
    reject: [],
    rejectLabelled: [
      { label: "wrong_model_pro_max", pattern: /프로\s*맥스|promax|pro\s*max|프맥/i },
      { label: "wrong_model_13_base_or_plus_or_mini", pattern: /아이폰\s*13\s*플러스|iphone\s*13\s*plus|아이폰\s*13\s*기본|아이폰\s*13\s*미니|iphone\s*13\s*mini/i },
      { label: "wrong_model_12", pattern: /아이폰\s*12|iphone\s*12/i },
      { label: "wrong_model_14", pattern: /아이폰\s*14|iphone\s*14/i },
      { label: "wrong_model_15", pattern: /아이폰\s*15|iphone\s*15/i },
      { label: "wrong_model_16", pattern: /아이폰\s*16|iphone\s*16/i },
      { label: "wrong_storage_256", pattern: /(?:^|[^0-9])256\s*(?:gb|기가)?\b/i },
  ...
  galaxy_buds_3_pro: {
    ...
    priceMin: 90_000,
    priceMax: 360_000,
    acceptAll: [/갤럭시\s*버즈|갤버즈|galaxy\s*buds/i],
    acceptAnyOf: [[/3\s*프로|3프로|3\s*pro|buds\s*3\s*pro|버즈\s*3\s*프로/i]],
    reject: [],
    rejectLabelled: [
      { label: "wrong_model_buds2_pro", pattern: /버즈\s*2\s*프로|버즈2\s*프로|버즈2프로|buds\s*2\s*pro|buds2\s*pro|buds2pro/i },
      { label: "wrong_model_buds2", pattern: /버즈\s*2\b|버즈2\b|buds\s*2\b|buds2\b/i },
      { label: "wrong_model_buds1", pattern: /버즈\s*1\b|버즈1\b|buds\s*1\b|buds1\b|갤럭시\s*버즈\s*기본|1세대\s*버즈/i },
      { label: "wrong_model_buds_live", pattern: /버즈\s*라이브|버즈라이브|buds\s*live|budslive/i },
      { label: "wrong_model_buds_fe", pattern: /버즈\s*fe|버즈fe|buds\s*fe|budsfe/i },
      { label: "wrong_model_buds_plus", pattern: /버즈\s*\+|버즈\+|버즈\s*플러스|buds\s*\+|buds\+|buds\s*plus/i },
      { label: "wrong_model_buds3_basic", pattern: /버즈\s*3\s*(?:기본|일반)|기본형\s*버즈\s*3/i },
      { label: "case_or_pouch_only", pattern: /케이스\s*(?:만|단품|개별)|파우치\s*만|하드\s*케이스\s*만|보관\s*케이스\s*만|커버\s*만/ },
      { label: "eartip_only", pattern: /이어\s*팁(?:\s*만|\s*단품|\s*교체)|이어팁\s*만|팁\s*만\s*판매/ },
      { label: "charger_or_cable_only", pattern: /충전기\s*만|케이블\s*만|usb\s*케이블\s*만|어댑터\s*만|크래들\s*만/i },
      { label: "single_unit_only", pattern: /왼쪽\s*만|오른쪽\s*만|좌측\s*만|우측\s*만|한쪽\s*만|편쪽\s*만|유닛\s*만|한짝/ },
  ...
  iphone_11_pro_128gb_self: {
    ...
    priceMin: 120_000,
    priceMax: 700_000,
    acceptAll: [/(?:아이폰|iphone)\s*11\s*(?:프로|pro)/i],
    acceptAnyOf: [],
    reject: [],
    rejectLabelled: [
      { label: "wrong_model_pro_max", pattern: /프로\s*맥스|promax|pro\s*max|프맥/i },
      { label: "wrong_model_plus", pattern: /아이폰\s*11\s*플러스|iphone\s*11\s*plus|플러스|\bplus\b/i },
      { label: "wrong_model_10_or_xs_xr", pattern: /아이폰\s*10(?!\d)|iphone\s*10(?!\d)|\bxs\b|\bxr\b/i },
      { label: "wrong_model_12", pattern: /아이폰\s*12(?!\d)|iphone\s*12(?!\d)/i },
      { label: "wrong_storage_64", pattern: /(?:^|[^0-9])64\s*(?:gb|기가)\b/i },
      { label: "wrong_storage_256", pattern: /(?:^|[^0-9])256\s*(?:gb|기가)?\b/i },
      { label: "wrong_storage_512_1tb", pattern: /512\s*(?:gb|기가)?|1\s*tb|1\s*테라/i },
      { label: "carrier_skt", pattern: /\bskt\b\s*(?:완납|개통|약정|이동|승계|전용)|skt\s*완납폰|skt\s*전용/i },
