...
async function main(): Promise<void> {
  ...
  for (const entry of registry.entries) {
    ...
    manifests.push({
      ...
      topMetrics: top,
      runtimeApprovedRows:
        typeof metrics.runtimeApprovedRows === "number" ? (metrics.runtimeApprovedRows as number) : null,
    });
  }
  ...
  const report = {
    ...
    decision: "phones_packet_manifest_report_only",
    totals,
    manifests,
  };

  await mkdir(reportsDir, { recursive: true });
  const jsonPath = path.join(reportsDir, "phones-packet-manifest-latest.json");
  await writeFile(jsonPath, JSON.stringify(report, null, 2));
  const md = [
    "# Phones Packet Manifest",
    "",
    `Generated: ${report.generatedAt}`,
    "",
    "Report-only manifest collecting top-line metrics for each phones packet.",
    "",
    "## Totals",
    "",
  ...
  console.log(
    `phones-packet-manifest: packets=${totals.packets}, runtimeApprovedNonZero=${totals.runtimeApprovedNonZero}, nonReportOnly=${totals.nonReportOnly}`,
  );
}
