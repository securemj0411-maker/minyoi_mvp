...
const TARGET = {
  skuId: "speaker-bose-soundlink-mini-ii",
  queries: [
...
const HARD_REJECT_KEYWORDS = [
  ...
  "도크만", "충전도크", "wireless charger", "charging dock",
  "부품용", "파손", "고장", "침수", "수리이력",
  "매입", "삽니다", "구해요", "구매합니다", "구합니다",
  // Bose SoundLink 다른 변형 격리
  "flex", "플렉스", "revolve", "리볼브", "color", "컬러", "micro", "마이크로",
  // Mini I (1세대) 격리
  "mini i ", "soundlink mini i ", "사운드링크 미니 i ", "mini1", "미니 1세대", "미니1세대",
  // 사운드바/QC
  "사운드바", "soundbar", "qc ", "quietcomfort", "qc ultra",
  // 타 브랜드
  "jbl", "marshall", "마샬", "boombox", "샤오미", "sonos", "소노스",
  "노래방", "karaoke",
];
...
async function main() {
  ...
  console.log(`existing Bose SoundLink Mini II pids: ${skipPids.size}`);
  ...
  for (const item of items.values()) {
    ...
    // require bose + soundlink mini + gen 2/II
    const hasBrand = norm.includes("bose") || norm.includes("보스");
    const hasSoundlinkMini = norm.includes("soundlinkmini") || norm.includes("사운드링크미니") || (norm.includes("soundlink") && norm.includes("mini")) || (norm.includes("사운드링크") && norm.includes("미니"));
    const hasGen2 = norm.includes("mini2") || norm.includes("mini 2") || norm.includes("miniii") || norm.includes("mini ii") || norm.includes("미니2") || norm.includes("미니 2") || norm.includes("미니ii") || norm.includes("미니 ii");
    if (!(hasBrand && hasSoundlinkMini && hasGen2)) continue;
  ...
  for (const cand of validated) {
    ...
    rawRows.push({
      ...
      query: `wave20_speaker_bose_soundlink_mini_ii_boost:${TARGET.skuId}`, source: "bunjang",
      ...
      raw_json: { source: "wave20_speaker_bose_soundlink_mini_ii_boost", lane: TARGET.skuId },
    });
  ...
  const report = {
    ...
    category: "wave20_speaker_bose_soundlink_mini_ii_boost",
    metrics: {
  ...
  const jsonPath = path.join(reportsDir, "wave20-speaker-bose-soundlink-mini-ii-boost-latest.json");
  await writeFile(jsonPath, JSON.stringify(report, null, 2));
  console.log(`wave20 bose soundlink mini ii: collected=${items.size}, validated=${validated.length}, applied=${applyMode ? rawRows.length : 0}, expectedKeys=${[...expectedKeys].join(",")}`);
}
