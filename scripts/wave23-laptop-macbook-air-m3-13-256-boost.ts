...
const HARD_REJECT_KEYWORDS = [
  ...
  "intel", "인텔", "i5", "i7", "i9",
  // Pro / 15"/16" 격리
  "macbook pro", "맥북 프로", "맥북프로", "mbp",
  "15인치", "15형", "15\"", "15in", "16인치", "16형", "16\"", "16in",
  // 다른 Mac 라인업
  "imac", "아이맥", "mac mini", "맥미니", "mac studio", "맥스튜디오", "mac pro", "맥프로",
  // 타 브랜드 노트북
  "lg 그램", "lg그램", "삼성 갤럭시 북", "갤럭시북", "갤럭시 북", "msi", "asus", "레노버", "hp 노트북",
  // 단품/액세서리
  "매직마우스만", "매직키보드만", "모니터만",
  // 다른 용량 격리 (M3 13 256 narrow lane이므로 512/1tb 제외)
  "512gb", "512g ", "1tb", "1테라", "2tb",
];
...
async function main() {
  ...
  console.log(`existing MacBook Air M3 13 256 pids: ${skipPids.size}`);
  ...
  for (const item of items.values()) {
    ...
    // require MBA + M3 + 13 (256 is recommended but optional - parser fills it)
    const hasMbAir = (norm.includes("맥북") && norm.includes("에어")) || (norm.includes("macbook") && norm.includes("air")) || norm.includes("맥북에어");
    const hasM3 = /\bm3\b|m3칩|m3맥북|m3에어|m3 8|m3 16|m3 256|m3 512/.test(item.name.toLowerCase());
    const has13 = norm.includes("13인치") || norm.includes("13형") || norm.includes("13\"") || norm.includes("13in") || /\bair13|에어13|13에어|13macbook|13맥북|macbookair13/.test(norm);
    if (!(hasMbAir && hasM3 && has13)) continue;
  ...
  for (const cand of validated) {
    ...
    rawRows.push({
      ...
      query: `wave23_laptop_macbook_air_m3_13_256_boost:${TARGET.skuId}`, source: "bunjang",
      ...
      raw_json: { source: "wave23_laptop_macbook_air_m3_13_256_boost", lane: TARGET.skuId },
    });
    parsedRows.push(toParsedListingRow(cand.pid, parsed));
  }

  if (applyMode && rawRows.length > 0) {
    if (process.env.WAVE23_WRITE_APPROVED !== "1") throw new Error("Apply refused: set WAVE23_WRITE_APPROVED=1");
  ...
  const report = {
    ...
    category: "wave23_laptop_macbook_air_m3_13_256_boost",
    metrics: {
  ...
  const jsonPath = path.join(reportsDir, "wave21-macbook-air-m3-13-256-boost-latest.json");
  await writeFile(jsonPath, JSON.stringify(report, null, 2));
  console.log(`wave23 macbook air m3 13 256: collected=${items.size}, validated=${validated.length}, applied=${applyMode ? rawRows.length : 0}, expectedKeys=${[...expectedKeys].join(",")}`);
}
