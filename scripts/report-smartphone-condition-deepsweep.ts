...
function negatedDamageContext(text: string, key: string) {
  ...
  if (key === "functional_component_issue") {
    return /네고불가|교환불가|환불불가|반품불가|택배거래희망|불량없|이상x|이상한소리(?:하면|하시면|하실|하는).{0,24}(?:답|차단|문의|연락|거래)|(?:스피커|마이크|통화|진동|버튼|볼륨|전원|충전|유심|와이파이|블루투스|소리|연결|작동|지문인식).{0,28}(?:정상|문제없|문제는없|문제1도없|큰문제는없|이상없|이상전혀없|전혀없|이상무|잘됨|잘됩니다|잘되는상태)|(?:확인|정확히확인).{0,16}불가.{0,24}(?:충전|배터리|성능)|충전.{0,24}(?:고민|문제).{0,12}없|(?:기능|전기능|모든기능).{0,18}(?:완벽하게작동|정상|문제없|이상없|전혀없|잘됨)|(?:모든문제|큰문제|문제|하자).{0,8}(?:없|없음|없습니다|없어요|없는)|문제\d*도없/.test(compact);
  }
  if (key === "display_panel_issue_broad") {
    return /무잔상|잔상(?:유무)?무|잔상없|잔상양호|(?:액정|화면|디스플레이|스크린).{0,18}(?:생활기스|기스|깨진거아니라|깨진것처럼보이는데필름|깨짐같은건필름|필름(?:이|만)?(?:깨진|깨짐|금)|필름붙)|(?:보호)?(?:필름|강화유리|유리필름).{0,18}(?:깨진|깨짐|금).{0,24}(?:액정|화면|디스플레이|스크린).{0,12}(?:아니|아님|아닙)/.test(compact);
  }
