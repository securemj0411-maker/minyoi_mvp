...
async function main() {
  const samples = JSON.parse(await readFile("category-intelligence/ipad_air_m2_11_256_wifi/parse_ready_sample.json", "utf-8"));
  const rows = Array.isArray(samples) ? samples : (samples as { samples?: unknown[] }).samples ?? [];
  let t=0,n=0,b=0,nu=0,w=0;
  const bIds: Record<string,number>={}, wIds: Record<string,number>={};
  ...
  for (const row of rows as Array<{name?:string;description?:string}>) {
    ...
    if (r?.laneKey==="ipad_air_m2_11_256_wifi") n++;
    else if (r?.laneKey) { w++; wIds[r.id]=(wIds[r.id]??0)+1; }
    else if (r) { b++; bIds[r.id]=(bIds[r.id]??0)+1; if(fails.length<5)fails.push({n:row.name??"",d:(row.description??"").slice(0,80),r:r.id}); }
  ...
  console.log(`Wrong:`); for (const [id,c] of Object.entries(wIds).sort((a,b)=>b[1]-a[1])) console.log(`  ${c}: ${id}`);
