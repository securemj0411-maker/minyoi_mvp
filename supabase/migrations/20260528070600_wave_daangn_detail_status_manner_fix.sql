-- Wave 904: Daangn search-only rows were inserted as detail_status='done'.
--
-- Fix the v2 bulk upsert so:
-- - search-only rows can be inserted as pending by the application;
-- - a later detail fetch can promote pending -> done and set detail_enriched_at;
-- - search-only re-seen rows never downgrade an already done row;
-- - manner temperature/review count keep the existing coalesce behavior.
...
begin
  with upserted as (
    ...
    on conflict (pid) do update set
      ...
      detail_status = case
        when excluded.detail_status = 'done' then 'done'
        else mvp_raw_listings.detail_status
      end,
      detail_enriched_at = case
        when excluded.detail_status = 'done' then coalesce(excluded.detail_enriched_at, mvp_raw_listings.detail_enriched_at)
        else mvp_raw_listings.detail_enriched_at
      end,
    ...
    where
      ...
      (excluded.detail_status = 'done' and mvp_raw_listings.detail_status is distinct from 'done') or
      (excluded.detail_status = 'done' and mvp_raw_listings.detail_enriched_at is distinct from excluded.detail_enriched_at) or
      mvp_raw_listings.pool_eligible is distinct from excluded.pool_eligible or
