-- Wave 2026-05-25: credit package keys.
-- Pricing now has single/trial/starter/plus/pro one-off credit packages.

alter table public.mvp_user_plans
  drop constraint if exists mvp_user_plans_plan_key_check;

alter table public.mvp_user_plans
  add constraint mvp_user_plans_plan_key_check
  check (plan_key in ('free','single','trial','starter','plus','pro'));
