...
test("beginner guide uses existing evidence without guaranteed-profit copy", () => {
  ...
  assert.doesNotMatch(modal, /introGuideStep/);
  assert.doesNotMatch(modal, /오늘 볼 만한 매물만 남겨뒀어요/);
  assert.doesNotMatch(modal, /step\.tone === "intro" \? <BeginnerGuideSafetyFilterNote card=\{card\} variant="intro" \/> : null/);
  ...
  assert.match(modal, /grid grid-cols-3 gap-1\.5/);
  assert.doesNotMatch(modal, /data-beginner-guide-channel-profit[\s\S]{0,220}grid grid-cols-1/);
  assert.match(modal, /당근 직거래/);
  assert.match(modal, /추천: \{preferDaangn \? "당근 먼저 등록" : "중고 마켓 먼저 등록"\}/);
  assert.match(modal, /안 팔리면 번개장터·중고나라로 넓히면 됩니다/);
  ...
  assert.match(modal, /확인하고 \{marketplaceLabel\} 보기/);
  ...
  assert.doesNotMatch(modal, /대신 확인한 것|추천 전에 걸러진 매물|단품·액세서리·업자성·모델 확인 필요|모델 확인 필요|표본이 부족하면 부족하다고 표시/);
