...
test("telegram GET admin decisions require signed action tokens", () => {
  ...
  assert.match(membershipDecide, /confirmHtml\(id, decision, actionUrl\)/);
  assert.match(membershipDecide, /handleTelegramTokenPost/);
  assert.match(membershipDecide, /form\?\.get\("confirm"\) !== "1"/);
