...
- 당근 로컬 prefetch는 가까운 지역 ID를 묶어서 조회하지 않고, 가까운 지역부터 1개씩 확인한다.
...
- 또한 가까운 지역 ID를 24개씩 묶어 raw listing을 `last_seen_at desc`로 가져오면, 최신글이 많은 지역이 가까운 동네 후보를 밀어내는 문제가 있었다.
