...
- 모바일 수직 공간을 줄이기 위해 상단 `deuktem membership`, `1/4` 헤더를 제거하고 첫 chunk 안의 핵심 카피/지도/지역 티오만 남겼다.
- 손으로 만든 가짜 시도 polygon은 폐기하고, Wikimedia Commons `Administrative divisions map of South Korea.svg` 기반의 실제 남한 행정구역 SVG 데이터를 별도 파일로 분리했다.
- 실제 시군구 경계가 보이는 SVG 면 지도 형태로 바꾸고, hover/선택 시 해당 지역이 진해지고 `n자리 남음` 팝업이 뜨도록 했다.
- 서울 25개 구, 경기 주요 시·군 등 하위 지역 티오 chip을 선택 지역별로 보여준다.
