...
- 신규 `DAANGN_SEARCH_REGION_SEEDS`: 6,333개 동·읍·면 leaf region
...
- `daangn-region-parents.json`도 6,333개 leaf full path로 갱신

## 검증

- 강남구 숨김 sibling인 `삼성동(6034)`, `개포동(6030)`, `도곡동(6033)`이 seed에 포함됨.
- 동작구 `상도동(6093)`, `상도1동(6092)`, `흑석동(331)`, `대방동(337)`이 모두 full path로 매핑됨.
- 세종처럼 `name2=null`인 단층 광역자치 region도 `조치원읍(1243)`, `종촌동(4153)` 등으로 매핑됨.
...
대신 rotation 대상이 274개 대표 지역에서 6,333개 실제 동네로 바뀌므로, 한 바퀴 도는 주기는 길어진다.
따라서 immediate ready 수가 폭증하기보다, 사당동/역삼동 같은 대표 동 편중이 빠지고 전국 동 단위 커버리지가 넓어지는 효과가 먼저 나온다.

## 보류

- 6,333개 전체 leaf 좌표를 Kakao로 정밀 생성하는 작업은 보류했다.
