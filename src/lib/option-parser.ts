...
function conditionFromText(
  ...
  const maintenanceReplacementPositive = /(?:그립|필터|브러쉬|브러시).{0,20}교체|그립\s*새로\s*감|새\s*(?:그립|필터|브러쉬|브러시)/.test(lower);
  const batteryReplacementPositive = /(?:새|신품|정품)?\s*(?:배터리|밧데리).{0,24}교체.{0,40}(?:효율|성능)\s*[:：]?\s*(?:100\s*%?|백\s*퍼)|(?:새|신품)\s*(?:배터리|밧데리).{0,16}교체.{0,90}(?:작동|상태|사용).{0,12}(?:양호|정상|좋|가능)|(?:배터리|밧데리).{0,16}(?:새것|새거|신품).{0,16}교체/.test(lower);
  const normalFunctionPositive = /모든\s*기능\s*(?:다\s*)?(?:정상\s*작동|정상작동|정상|잘\s*(?:됨|됩니다|작동)|이상\s*없|문제\s*없)|전\s*기능\s*(?:정상|이상\s*없|문제\s*없)|전기능\s*(?:정상|이상\s*없|문제\s*없)/.test(lower);
  if (
    !noRepairOrDefect &&
    !maintenanceReplacementPositive &&
    !batteryReplacementPositive &&
    !normalFunctionPositive &&
    /수리|교체|하자|고장|불량|파손|깨짐/.test(defectRiskText)
...
export function conditionFromTextFashion(
  ...
  const add = (note: string, delta: number) => {
    ...
    // 갑피/앞코 찢김 — historical parsed sweep 에서 "앞에 찢김, 수선 가능" 누락 발견.
    // 단순 "수선 맡길 때 필요" 같은 AS 안내는 손상으로 보지 않도록 위치/파손어 근접 조건을 둔다.
    const noShoeUpperDamage = /(?:박스|속포장지|포장지|더스트백|쇼핑백|가방).{0,24}(?:찢|터짐|터진|뜯|튿|깨짐|깨진)|(?:찢|터짐|터진|뜯|튿|깨짐|깨진).{0,24}(?:박스|속포장지|포장지|더스트백|쇼핑백|가방)|(?:찢어진|찢어짐|찢김|찢어져|뜯어짐|뜯어진|튿어짐|튿어진|터짐|터진|깨짐|깨진|갈라짐).{0,8}(?:없|없음|없습니다|없어요|없는|없고|아님)/.test(lower);
    if (
      !noShoeUpperDamage &&
      (
        /(?:왼쪽\s*신발|오른쪽\s*신발|신발|새끼발가락|사이드|옆에|옆쪽?|측면|안쪽|뒤쪽?|뒷쪽|뒤꿈치|뒷꿈치|뒷축|걸이부분|앞|앞쪽|앞코|토박스|어퍼|갑피|발등|아웃솔|밑창|솔).{0,28}(?:찢김|찢어짐|찢어진|찢어져|찢어|터짐|터진|뜯김|뜯어짐|뜯어진|튿어짐|튿어진|튿어져|벌어짐|갈라짐|깨짐|깨진|수선\s*가능|수선\s*필요|보강\s*추천|밑창보강)/.test(lower) ||
        /(?:찢어진\s*부분|찢어진부분|뜯어진\s*부분|뜯어진부분|뜯어짐|튿어짐|터진\s*부분|터진부분|터짐|깨짐).{0,12}(?:있|있음|있어요|있습니다|감안|사진|첨부)/.test(lower)
      )
    ) {
      add("shoe_upper_structural_damage", -0.2);
      if (!notes.includes("repair_or_defect_signal")) {
        notes.push("repair_or_defect_signal");
      }
    }
    // 오염/이염/얼룩 — 신발 ready sweep 에서 "약간의 얼룩있어", "이염 있습니다" 변형 누락 발견.
    const noShoeStain = /(?:박스|속포장지|포장지).{0,16}(?:오염|얼룩|이염|변색|황변)|(?:오염|얼룩|이염|변색|황변)(?:\s*및\s*(?:하자|데미지))?\s*(?:없|없음|없습니다|없어요|없고|없이|아님|깨끗|하나\s*없|전혀\s*없)|(?:오염|얼룩|이염|변색|황변)\s*(?:도\s*)?아니|(?:오염|얼룩|이염|변색|황변)\s*외(?:에|로)?\s*(?:깨끗|없|괜찮)|변색\s*위험/.test(lower);
    if (
      !noShoeStain &&
      /(?:신발끈|끈|앞코|밑창|중창|갑피|어퍼|안쪽|목\s*부분|뒤쪽|뒷쪽|뒤꿈치|힐컵|토박스|고무\s*부분|고무|스웨이드|패브릭|가죽|아웃솔|인솔|내부|바닥).{0,22}(?:오염|얼룩|이염|변색|황변|때\s*탐|때탐|생활\s*오염|생활오염|떼\s*탄)|(?:살짝|약간|미세|작은|소량|부분적?)?\s*(?:오염|얼룩|이염|변색|황변|생활\s*오염|생활오염)(?:이|가|은|는)?\s*(?:있|있음|있습니다|있어요|있지만|있어|있네요|잇|보임|보입니다|남아|자국|흔적|생김|생긴|생깁|발생|존재|조금|살짝|약간|확인|되어|된|온\s*듯|끼|사진\s*참고|\/)|(?:오염|얼룩|이염|변색|황변|생활\s*오염|생활오염).{0,14}(?:있|있음|있습니다|있어요|있지만|있어|있네요|잇|보임|보입니다|남아|자국|흔적|생김|생긴|생깁|발생|존재|조금|살짝|약간|확인|되어|된|온\s*듯|끼|사진\s*참고|\/)|(?:약간의|미세한|작은)\s*(?:오염|얼룩|이염|변색|황변)|(?:오염|얼룩|이염|변색|황변)(?:\s|$)/.test(lower)
    ) {
    ...
    const noShoeHygiene = /(?:악취|냄새|발냄새|담배\s*냄새|곰팡이).{0,10}(?:없|없음|없습니다|없어요|없는|아님)|(?:없|없는|없고).{0,10}(?:악취|냄새|발냄새|담배\s*냄새|곰팡이)/.test(lower);
    if (!noShoeHygiene && /악취|발냄새|담배\s*냄새|곰팡이/.test(lower)) {
      add("shoe_hygiene_warning", -0.15);
      if (!notes.includes("cosmetic_wear")) {
        notes.push("cosmetic_wear");
      }
    }
  ...
  if (category === "bag") {
    ...
    const noLeatherDamage = /가죽\s*(?:까짐|벗겨|벚겨|갈라|뜯|헤짐|해짐)\s*(?:없|없음|아님)/.test(lower);
    if (!noLeatherDamage && /가죽\s*(?:까짐|벗겨|벚겨|벗겨짐|벚겨짐|갈라짐|찢어|뜯김|뜯어짐|헤짐|해짐|크랙|박리)|레더\s*(?:까짐|벗겨|벚겨|갈라|뜯|헤짐|해짐|크랙)|코팅\s*(?:벗겨|벚겨|박리|들뜸)|바닥(?:이|은|는)?\s*(?:좀\s*)?(?:헤졌|헤짐|해짐)|(?:테두리|상부|가죽|원단|표면|부분).{0,24}(?:데미지|손상)|(?:데미지|손상).{0,18}(?:있|있음|있습니다|있으니|사진|확인)/.test(lower)) {
    ...
    // 손잡이 마모/끊어짐/늘어남.
    if (/손잡이.{0,16}(?:마모|닳|끊어|늘어|찢어|찢김|찢어짐|벗겨|페인팅\s*벗)|핸들(?:부분)?.{0,16}(?:마모|닳|끊어|늘어|찢어|찢김|찢어짐)|(?:스트랩|어깨끈|크로스\s*끈|끈).{0,18}(?:끊어|찢어|벗겨|수선\s*(?:필요|해야|가능)|분실|없음|빠짐|올\s*풀림)|똑딱이.{0,10}(?:빠짐|빠져|분실)|(?:그물|망사).{0,18}(?:튿어짐|튿어진|뜯김|뜯어짐|찢어)/.test(lower)) {
      add("bag_handle_worn", -0.15);
    }
    // 안감/내부/이너백/가죽 표면 오염·이염·태닝 — historical parsed sweep 누락 보강.
    // 끈적/찢어짐처럼 구조 결함은 아니지만, 시세 비교와 UI chip 에 반드시 보여줘야 하는 감가 신호.
    const noBagStain = /(?:오염|얼룩|이염|변색|황변|태닝|때탐|물\s*얼룩|색\s*바램|색바램|색\s*빠짐|펜\s*자국|볼펜\s*자국|잉크\s*자국)(?:\s*및\s*(?:하자|데미지))?\s*(?:없|없음|없습니다|없어요|없고|없이|아님|깨끗|전혀\s*없)|(?:오염|얼룩|이염|변색|황변|태닝|때탐|색\s*바램|색바램|색\s*빠짐)\s*외(?:에|로)?\s*(?:깨끗|없|괜찮)|(?:이염|오염|얼룩).{0,8}(?:방지|예방)|(?:오염|스크래치).{0,12}강한|(?:색\s*바램|색바램)\s*아닌/.test(lower);
    if (
      !noBagStain &&
      /(?:안감|내부|이너백|밑면|바닥|바디|어깨끈|스트랩|플랩|모서리|코너|가죽|원단|앞판|뒷판|로고|끈|외부|손잡이|자크|지퍼|메탈).{0,24}(?:오염|얼룩|이염|변색|황변|때탐|태닝|물\s*얼룩|색\s*바램|색바램|색\s*빠짐|노랗게|펜\s*자국|볼펜\s*자국|잉크\s*자국)|(?:생활\s*)?(?:오염|얼룩|이염|변색|황변|때탐|태닝|물\s*얼룩|색\s*바램|색바램|색\s*빠짐)(?:들)?(?:이|가|은|는|도)?\s*(?:있|있음|있어요|있습니다|있지만|있어|보임|보입니다|보이는|남아|자국|흔적|진행|조금|약간|살짝|다소|소량|감안|존재|생김|생긴|느껴|보여|참고|확인|외에|부분|정도|많|좀|되|된|돼|상태)|(?:화장품|펜|볼펜|잉크).{0,16}(?:오염|얼룩|이염|자국|자욱)|(?:펜\s*자국|볼펜\s*자국|볼펜잉크\s*자욱|볼펜잉크자욱|잉크\s*자국|잉크자국)|(?:손\s*떼|손때|때)\s*탄|노랗게\s*이염/.test(lower)
    ) {
      add("bag_stain_or_discoloration", -0.12);
      if (!notes.includes("cosmetic_wear")) {
        notes.push("cosmetic_wear");
      }
    }
    const noBagHygiene = /(?:냄새|악취|담배\s*냄새)(?:가|이|는|은)?\s*(?:없|없음|없습니다|없어요|안\s*남|안납니다|깨끗)/.test(lower);
    if (!noBagHygiene && /(?:보관\s*)?(?:냄새|악취|담배\s*냄새)(?:가|이|는|은)?\s*(?:있|있음|있습니다|있어요|남|납니다|나요|심|조금|약간)/.test(lower)) {
      add("bag_hygiene_warning", -0.12);
      if (!notes.includes("cosmetic_wear")) {
        notes.push("cosmetic_wear");
      }
    }
  ...
  if (category === "clothing") {
    ...
    if (
      !noStructuralDamage &&
      /(?:작은\s*)?데미지\s*(?:있|있음|\d+|두\s*개|여러|조금|살짝)|구멍\s*(?:있|있음|\d+|작게|작은)|미세\s*구멍|찢어(?:짐|졌|진)|찢김|해짐|헤짐|터짐|터진\s*곳|터진곳|튿어짐|튿어진|뜯어짐|뜯어진|올\s*풀림|박음질만|보강\s*(?:필요|해야|요망)|수선\s*(?:필요|해야|요망)/.test(lower)
    ) {
    ...
    // 인쇄/프린팅 갈라짐 — 그래픽 티 흔함.
    if (/(?:인쇄|프린팅|프린트|로고).{0,20}(?:갈라|벗겨|벗겨짐|벗겨진|까짐|까져|박리|찢어|지워짐|지워져)/.test(lower)) {
      add("clothing_print_cracked", -0.1);
    }
    // 얼룩 — clothing 만 (shoe 는 c_grade 의 얼룩 패턴 있음).
    const noStain = /(?:오염|얼룩|이염|생활\s*오염|생활오염)(?:\s*및\s*(?:하자|데미지))?\s*(?:없|없음|없습니다|없어요|없고|없이|아님|깨끗|하나\s*없|전혀\s*없)|(?:오염|얼룩|이염)\s*외(?:에|로)?\s*(?:깨끗|없|괜찮)|(?:이염|오염|얼룩).{0,8}(?:방지|예방)/.test(lower);
    if (
      !noStain &&
      /(?:살짝|약간|미세|작은|소량|부분적?|연한|옅은)?\s*(?:생활\s*)?(?:오염|얼룩|이염|생활오염)(?:이|가|은|는|도)?\s*(?:있|있음|있습니다|있어요|있지만|있어|있네요|보임|보입니다|남아|자국|흔적|심|많|크|존재|생김|생긴|조금|약간|살짝|참고|확인|상대방|구매거절|저렴)|(?:오염|얼룩|이염|생활오염).{0,18}(?:가슴|팔|허벅지|엉덩이|소매|카라|목\s*택|목택|마지막|사진|참고|확인|조금|약간|살짝|있|존재|상대방|구매거절|저렴|제외)|(?:약간의|미세한|작은|연한|옅은)\s*(?:오염|얼룩|이염)|(?:커피|카라|목\s*택|목택|소매|겨드랑이|앞|뒤|뒷면|등판|가슴|팔|허벅지|엉덩이|전면부).{0,18}(?:오염|얼룩|이염)|(?:오염|얼룩|이염)\s*같/.test(lower)
    ) {
