...
export function conditionFromTextFashion(
  ...
  if (category === "bag") {
    ...
    const noLeatherDamage = /가죽\s*(?:까짐|벗겨|벚겨|갈라)\s*(?:없|없음|아님)/.test(lower);
    if (!noLeatherDamage && /가죽\s*(?:까짐|벗겨|벚겨|벗겨짐|벚겨짐|갈라짐|찢어|크랙|박리)|레더\s*(?:까짐|벗겨|벚겨|갈라|크랙)|코팅\s*(?:벗겨|벚겨|박리|들뜸)/.test(lower)) {
    ...
    // 손잡이 마모/끊어짐/늘어남.
    if (/손잡이.{0,12}(?:마모|닳|끊어|늘어|찢어|찢김|찢어짐|벗겨|페인팅\s*벗)|핸들(?:부분)?.{0,12}(?:마모|닳|끊어|늘어|찢어|찢김|찢어짐)|스트랩.{0,12}(?:끊어|찢어|벗겨)/.test(lower)) {
    ...
    const noBagStain = /(?:오염|얼룩|이염|변색|황변|태닝|때탐|물\s*얼룩|색\s*바램|색바램|색\s*빠짐)(?:\s*및\s*(?:하자|데미지))?\s*(?:없|없음|없습니다|없어요|없고|없이|아님|깨끗|전혀\s*없)|(?:오염|얼룩|이염|변색|황변|태닝|때탐|색\s*바램|색바램|색\s*빠짐)\s*외(?:에|로)?\s*(?:깨끗|없|괜찮)/.test(lower);
    if (
      !noBagStain &&
      /(?:안감|내부|이너백|밑면|바디|어깨끈|스트랩|플랩|모서리|코너|가죽|원단|앞판|뒷판).{0,24}(?:오염|얼룩|이염|변색|황변|때탐|태닝|물\s*얼룩|색\s*바램|색바램|색\s*빠짐|노랗게)|(?:오염|얼룩|이염|변색|황변|때탐|태닝|물\s*얼룩|색\s*바램|색바램|색\s*빠짐)(?:이|가|은|는)?\s*(?:있|있음|있어요|있습니다|있지만|있어|보임|보입니다|남아|자국|흔적|진행|조금|약간|살짝|존재|생김|생긴)|노랗게\s*이염/.test(lower)
    ) {
  ...
  if (category === "clothing") {
    ...
    if (
      !noStructuralDamage &&
      /(?:작은\s*)?데미지\s*(?:있|있음|\d+|두\s*개|여러|조금|살짝)|구멍\s*(?:있|있음|\d+|작게|작은)|찢어(?:짐|졌|진)|찢김|해짐|헤짐|터짐|튿어짐|튿어진|뜯어짐|뜯어진|보강\s*(?:필요|해야|요망)|수선\s*(?:필요|해야|요망)/.test(lower)
    ) {
    ...
    if (
      !noStain &&
      /(?:살짝|약간|미세|작은|소량|부분적?)?\s*(?:오염|얼룩|이염|생활\s*오염)(?:이|가|은|는)?\s*(?:있|있음|있습니다|있어요|있지만|있어|있네요|보임|보입니다|남아|자국|흔적|심|많|크|존재|생김|생긴)|(?:약간의|미세한|작은)\s*(?:오염|얼룩|이염)|(?:커피|카라|목\s*택|목택|소매|겨드랑이|앞|뒤|뒷면|등판).{0,18}(?:오염|얼룩|이염)|(?:오염|얼룩|이염)\s*같/.test(lower)
    ) {
