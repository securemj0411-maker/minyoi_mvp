...
export function parseConditionTier(text: string): ConditionTier {
  ...
  // 부적격 (reject) — 명시 손상
  // Wave 151: 본드 수리, 깔창 분실 추가
  if (/파손|크랙|찢어짐|구멍|얼룩\s*심함|변색\s*심함|곰팡이|악취|냄새\s*심함|손상|수리\s*필요|수선\s*필요|찌그러짐|변형\s*심함|밑창\s*벗겨|본드로\s*붙여|깔창\s*분실|내부\s*깔창\s*분실|튿어짐\s*있어서|찢어짐\s*있어서/.test(t)) return "reject";
