...
export function parseConditionTier(text: string): ConditionTier {
  ...
  if (
    /미개봉|봉인|택\s*달[린림힌]|택\s*부착|택\s*그대로|tag\s*on|tagon|박스컷|나코탭|kream\s*택|크림\s*택|크림\s*인증|풀박스\s*\+?\s*택|박스\s*\+?\s*택|택\s*있/.test(t)
  ) return "s_grade";
  // 셀러 표기 — 1단계 깎음
  ...
  if (
    /미사용|미착용|미시착|새상품(?!\s*[abc])|민트|한\s*번도\s*안\s*(신음|입음|멤|탐|사용|신|입|착)|신은?\s*적\s*없|신어\s*본?\s*적\s*없|신지\s*않은|신지를\s*않|착용해본\s*적\s*없|착용한\s*적\s*없|박스\s*그대로|박스채로|포장\s*그대로|포장\s*그대로|박스\s*개봉\s*안|풀박\s*새상품|보관만|예비용으로\s*구매|구입만\s*하고\s*보관|신상품|매장\s*정품|거의\s*(새|안\s*신|안\s*입|안\s*들|안\s*탐|새것|새신발)|1\s*[~-]?\s*2\s*번\s*(신|입|들|탐)|한\s*두?\s*번\s*(신|입|착)|한두번|시착\s*(만|함)|시신음|착용\s*해?\s*본다고|사이즈\s*(미스|실패|안\s*맞|틀)|실착\s*[1-3]\s*회|단\s*[1-3]\s*회|구매\s*후\s*[1-3]\s*회|보관용|잠깐\s*(신|착용|시\s*신|밖에\s*안)|1\s*회\s*시착|선물용으로\s*받|실내에서만\s*신/.test(t)
  ) return "a_grade";
  // B급 (사용감 적음): 4-9회 단독 착용 / 상태 양호 / 컨디션 좋 / 9-10/10
  if (
    /사용감\s*적|약간\s*사용|잔기스|컨디션\s*좋|상태\s*양호|상태양호|상태\s*좋|상태좋|상태\s*우수|컨디션\s*우수|excellent|excellent\s*condition|9\s*\/\s*10|10\s*\/\s*10|(?:^|[^0-9])[4-9]\s*회\s*[가-힣\s]{0,5}?(신|착|입)|깨끗|깔끔|관리\s*잘|사용감\s*거의\s*없|사용감\s*없|마모\s*없|마모\s*거의\s*없|기스\s*없|기스\s*거의\s*없|외관\s*깨끗|외부\s*깨끗|상태\s*깨끗/.test(t)
  ) return "b_grade";
  // C급 (사용감 많음): 사용감 있/오염/스크레치/10회+ 착용
  if (
    /사용감\s*있|사용감\s*좀\s*있|많이\s*사용|보풀|색바램|변색|약간의?\s*오염|약간의?\s*얼룩|약간의?\s*스크\w*|앞코\s*스크\w*|미드솔\s*오염|밑창\s*닳|밑창\s*마모|(?:^|[^0-9])[1-9][0-9]\s*회\s*[가-힣\s]{0,5}?(신|착|입)|발자국|기스\s*있|스크래치|스크레치|굽\s*닳|굽이\s*닳|굽\s*마모|사용\s*흔적|뒤꿈치\s*닳/.test(t)
  ) return "c_grade";
  return null;
}
