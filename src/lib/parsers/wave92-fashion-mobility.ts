...
export function parseConditionTier(text: string): ConditionTier {
  ...
  const hasCSignal = !isSellerDisclaimer && /사용감(?:\s*있|\s*많|\s*좀\s*있|이?\s*많|은?\s*많|이?\s*있|은?\s*있)|착용감\s*있|약간의?\s*오염|얼룩|스크래치|스크레치|기스\s*있|기스\s*[가-힣\s]{0,5}?있|코부위\s*기스|튿어짐|갈라짐|헤짐|로고\s*지워|많이\s*신|많이\s*사용|뒷굽\s*사용|밑창\s*닳|밑창\s*마모|뒤꿈치\s*닳|구멍\s*하나|보풀\s*있|보풀이?\s*있|신발끈\s*보풀|물\s*빠짐|생활\s*먼지/.test(t);
