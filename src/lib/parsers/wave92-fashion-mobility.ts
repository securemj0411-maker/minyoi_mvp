...
export function parseShoeSizeMm(text: string): number | null {
  ...
  // 보수적: 기본은 "EU" prefix 필요. Wave 414 에서 명품 신발의 [40] / 42.5사이즈 / 후행 43 보강.
  const euMatch =
    text.match(/(?:eu|eur|유로|유럽|europe)\s*(3[5-9](?:\.5)?|4[0-6](?:\.5)?)/i)
...
//     clothing: hoodie / hoodie_zip / crewneck / tee / long_sleeve_tee / shirt / jacket / coat / down_jacket / pants / jeans /
...
function parseClothingProductType(text: string): ClothingProductType {
  ...
  // Wave 413: shirt-specific phrases must win before hoodie/tee fallbacks.
  // Thom Browne and designer shirts often include "반팔셔츠", "옥스포드 짚업셔츠",
  // or "후드 옥스포드 셔츠"; these are shirt lanes, not tee/hoodie lanes.
...
function parseBagProductType(text: string): BagProductType {
  ...
  // Wave 411: Bottega Cassette bag descriptions often mention included shopping bag/pouch
  // ("쇼핑백/파우치 등 구성품"). Keep the model bag as crossbody instead of fragmenting to tote/pouch.
  if (
    /(?:보테가|bottega|bottega veneta|보테가베네타).*(?:카세트 ?백|cassette ?bag|크로스 ?백|crossbody|cross ?bag)/.test(t) ||
    /(?:카세트 ?백|cassette ?bag|크로스 ?백|crossbody|cross ?bag).*(?:보테가|bottega|bottega veneta|보테가베네타)/.test(t)
  ) return "crossbody";
  ...
  // Wave 613: Hermes 시그니처 모델명 — title 우선 매칭 (description '지갑/장지갑' false positive 차단).
  // 알린 (Aline) = 크로스백/숄더, 시네틱 (Cinetique) = 숄더백.
  if (/알린백|aline\b|시네틱|cinetique|cinétique/.test(t)) return "shoulder";
  // Wave 620: Balenciaga Le Cagole = 핸드백/숄더 (card_holder 매칭 차단).
  if (/르 ?카골|le ?cagole|cagole/.test(t)) return "shoulder";
  // Wave 650: Prada Brique = 핸드백/숄더 (pouch 매칭 차단).
  if (/브리크|brique/.test(t)) return "shoulder";
  // tote — 탑핸들/핸드백.
...
function parseShoeProductType(text: string): ShoeProductType {
  ...
  // Wave 610: "tabi shoes" / "타비 슈즈" English suffix variant → sneaker (Wave 557 commented defaultProductType fallback에 의존했으나, defaultProductType 미박힌 SKU 일부는 type_unknown 잔존).
  if (/(?:타비|tabi)\s*(?:shoes?|슈즈)\b/.test(t)) return "sneaker";
...
const PARSER_VERSION_W92_SHOE_V8 = "wave92-shoe-v23";
...
const PARSER_VERSION_W216_CLOTHING_LATEST = "wave216-clothing-v31";
...
export function parseFashionMobility(input: ParseInput): ParsedListingOptions {
  ...
  return {
    ...
    parserVersion:
      category === "clothing"
        ? PARSER_VERSION_W216_CLOTHING_LATEST
        : category === "shoe"
