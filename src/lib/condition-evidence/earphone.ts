...
export function parseEarphoneConditionEvidence(input: {
  ...
  const audioEvidence = firstEvidence(sources, [
    /지지직|잡음|화이트\s*노이즈|소리.{0,18}(?:안\s*나|안나|안\s*들|작|먹먹|끊|튀|깨짐|이상|문제)|(?:한쪽|왼쪽|오른쪽).{0,18}(?:안\s*들|소리\s*안|소리\s*작)|음질.{0,16}(?:이상|문제|깨|먹먹)/,
  ]);
  const audioNegated = hasAny(sources, [
    /(?:소리|음질|스피커|좌우).{0,18}(?:이상\s*없|문제\s*없|정상|잘\s*됨|잘\s*나)/,
  ]);
  if (!audioNegated) {
  ...
  const ancEvidence = firstEvidence(sources, [
    /(?:노캔|노이즈\s*(?:캔슬링|켄슬링|캔슬|켄슬)|anc|주변음|투명\s*모드).{0,28}(?:불량|고장|문제|이상|안\s*됨|안됨|작동\s*안|지지직|잡음|먹통)/,
    /(?:불량|고장|문제|이상|안\s*됨|안됨|작동\s*안|지지직|잡음|먹통).{0,28}(?:노캔|노이즈\s*(?:캔슬링|켄슬링|캔슬|켄슬)|anc|주변음|투명\s*모드)/,
  ]);
  const ancNegated = hasAny(sources, [
    /(?:노캔|노이즈\s*(?:캔슬링|켄슬링|캔슬|켄슬)|anc|주변음|투명\s*모드).{0,20}(?:정상|문제\s*없|이상\s*없|잘\s*됨|잘\s*됩니다)/,
  ]);
  ...
  const physicalEvidence = firstEvidence(sources, [
    /깨졌|깨짐|파손|크랙|금\s*갔|부러|휘어|침수|찢어|찍힘\s*심|떨어(?:뜨|트)림|떨어트(?:려|린)|안\s*닫|안닫|닫힘\s*불량|닫히지\s*(?:않|안)/,
    /(?:충전|힌지|유닛|본체|케이스).{0,18}(?:깨|파손|크랙|부러|안\s*닫|안닫)/,
  ]);
  const physicalNegated = hasAny(sources, [
    /(?:깨짐|파손|크랙|찍힘|침수|수리|하자).{0,16}(?:없|없음|없습니다|아님)|(?:기스|스크래치).{0,12}(?:없|없음|없습니다)/,
  ]);
  if (!physicalNegated) {
