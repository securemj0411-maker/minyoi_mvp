  ...
  for (const row of sortedRows) {
    ...
    if (row.descriptionPreview && typeof row.descriptionPreview === "string") {
      ...
      const AD_PATTERNS = [
        ...
        // Wave 198 (2026-05-18): 의류 카테고리 특화 가품 광고 패턴.
        //   replica grade 표기 — 가품 셀러 frequent. 정상 매물은 거의 안 씀.
        /S\s*급\s*(?:미러|레플리카|급)/i,             // "S급 미러" / "S급 레플리카"
        /\b(?:rep|replica|미러급|미러 급)\b/i,        // rep / replica / 미러급
        /복각|이미테이션|imitation/i,                  // 한국 가품 표현
        // 광고 가품 셀러 — 의류 frequent
        /고퀄리티\s*복각/,                            // "고퀄리티 복각"
        /오프\s*화이트\s*공구/,                       // "오프화이트 공구" (가품 셀러)
        /택\s*그대로\s*보관/,                         // "택 그대로 보관" (광고 짧은 desc)
        // Wave 718 (2026-05-23): pool sample audit 발견 — 럭셔리 리테일 셀러 + 개인 대량 셀러 패턴.
        // sample: shoe-hermes-broad 에르메스 제트 스니커즈 170만 + 에르메스 오즈 뮬 156만 / bag-gucci-broad 140만 / macbook-pro 5건 동일 desc.
        // 럭셔리 리테일 셀러 시그니처
        /\*\s*실재고\s*보유중/,                       // "*실재고 보유중*" (별표 강조)
        /\*\s*모든제품\s*퀵\s*가능/,                  // "*모든제품 퀵 가능*"
        /매장가\s*[:：]\s*\d{2,3}\s*만/,              // "매장가 : 180만원"
        /\b-?\s*N\s*[:：]\s*새상품.{0,40}-?\s*S\+\s*[:：]/, // N:새상품 ... S+: 컨디션 등급표
        /제품\s*컨디션\s*기준표/,                     // "*제품 컨디션 기준표*"
        /가품일?\s*시.{0,10}(200|1000|10000)\s*%\s*(?:보상|배상|환불)/, // 가품일시 200%/1000% 보상
        // 개인 대량 셀러 — 안전거래 거부 + 전화번호 우회
        /안전\s*(?:거래|결제)\s*불가/,                // "안전거래 불가" / "안전결제 불가"
        /얼굴\s*보고\s*직거래만/,                     // "얼굴보고 직거래만"
        /이상한\s*쇼핑몰\s*포인트/,                   // "이상한 쇼핑몰 포인트 소리하면"
        /공\s*[01]\s*공\s+\d-\d-\d-\d/,               // "공1공 4-3-3-9" 전화번호 우회 표기
        // 럭셔리 명품 표 형식 listing (성별/상품명/사이즈/구성품/매장가 세트)
        /-?\s*성별\s*[:：].{0,40}-?\s*상품명\s*[:：]/, // "-성별: ... -상품명:"
        /-?\s*구성품\s*[:：].{0,40}-?\s*매장가\s*[:：]/, // "-구성품: ... -매장가:"
      ];
