        ...
        <div className="fixed inset-0 z-[9990] flex items-center justify-center overflow-y-auto bg-black/62 px-3 py-[calc(env(safe-area-inset-top)+16px)] backdrop-blur-sm sm:py-8">
          ...
          <div className="relative border-b border-blue-100 bg-[#f5f8ff] px-4 py-4 dark:border-zinc-800 dark:bg-white/6 sm:px-7 sm:py-5">
            <div className="relative flex items-start justify-between gap-4">
              ...
              {showReservationCountdown ? (
                <UrgencyCountdownCard
                  label="자리 예약 만료까지"
                  value={countdownLabel(reservationMsLeft)}
                  caption="시간 내 입금하지 않으면 예약이 취소돼요"
                />
              ) : null}
            </div>
          </div>
          <div className="px-3 py-3 sm:px-5 sm:py-4">
            ...
            <MembershipCheckoutBody
              ...
              depositState={
                depositNotifyState === "error" ? "idle" : depositNotifyState
              }
              autoApproveMsLeft={autoApproveMsLeft}
              copyOk={copyOk}
              busy={isBusy}
              onChooseToss={chooseTossPayment}
              onChooseBank={() => setSelectedPaymentMethod("bank")}
              onReopenToss={() => openTossSend(priceKrw)}
              onSwitchToBank={() => setSelectedPaymentMethod("bank")}
              onResetMethod={() => setSelectedPaymentMethod(null)}
              onCopyAccount={() => void copyAccountNumber()}
              onNotifyDeposit={() => void notifyDepositDone()}
              note={depositNotifyMessage}
              noteTone={depositNotifyState === "error" ? "error" : "info"}
            />
            ...
            {!renewalMode ? (
              <div className="mb-3 rounded-[14px] border border-emerald-400/22 bg-emerald-500/8 px-3 py-2.5 text-center">
                <div className="break-keep text-[12px] font-black text-emerald-200 sm:text-[13px]">
                  💰 한 매물당 평균 차익 <span className="text-emerald-100">약 4만원</span> · 매물 1~2건만 거래해도 멤버십값 회수
                </div>
