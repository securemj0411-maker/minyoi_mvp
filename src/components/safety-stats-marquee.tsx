...
function StatRow({ label, count }: { label: string; count: number }) {
  ...
  return (
    <div className="flex items-center justify-between gap-2">
      <span className="text-blue-800/85 dark:text-blue-300/85">{label}</span>
...
function GroupHeader({ label }: { label: string }) {
  return (
    <div className="mt-1.5 border-t border-blue-200/60 pt-1.5 text-[9px] font-black uppercase tracking-wider text-blue-700 dark:border-blue-900/60 dark:text-blue-400">
      {label}
    </div>
  );
}
...
export default function SafetyStatsMarquee() {
  ...
  if (!stats) {
    return (
      <div className="border-b border-blue-100 bg-blue-50/60 dark:border-blue-900/60 dark:bg-blue-950/30">
        <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-3 px-4 py-1.5">
          <span className="flex items-center gap-1.5 text-[11px] font-bold text-blue-800/70 dark:text-blue-300/70">
            <span className="text-blue-600/70 dark:text-blue-400/70"><ShieldIcon /></span>
            <span>오늘 득템잡이가 차단한 매물:</span>
            <span className="inline-block h-3 w-12 animate-pulse rounded bg-blue-200/80 align-middle dark:bg-blue-900/60" />
          </span>
          <span className="flex items-center gap-0.5 rounded-full bg-blue-600/40 px-2 py-0.5 text-[9px] font-black text-white/70">
            상세
  ...
  return (
    <div className="border-b border-blue-100 bg-blue-50/60 dark:border-blue-900/60 dark:bg-blue-950/30">
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between gap-3 px-4 py-1.5 transition-colors hover:bg-blue-100/60 dark:hover:bg-blue-900/30">
        <button
          type="button"
          onClick={() => setShowDetail((s) => !s)}
          className="flex min-w-0 flex-1 items-center gap-1.5 text-left text-[11px] font-bold text-blue-800 dark:text-blue-300"
          aria-expanded={showDetail}
        >
          <span className="text-blue-600 dark:text-blue-400"><ShieldIcon /></span>
        ...
        <div className="flex shrink-0 items-center gap-1">
          <button
            type="button"
            onClick={() => setShowDetail((s) => !s)}
            className="flex items-center gap-0.5 rounded-full bg-blue-600/90 px-2 py-0.5 text-[9px] font-black text-white"
          >
            {showDetail ? "접기" : "상세"}
          ...
          <button
            type="button"
            onClick={hideForWeek}
            className="flex items-center gap-0.5 rounded-full bg-white/70 px-2 py-0.5 text-[9px] font-black text-blue-800 hover:bg-white dark:bg-zinc-900/70 dark:text-blue-200"
      ...
      {showDetail && (
        <div className="mx-auto max-w-6xl border-t border-blue-200/70 bg-blue-50/40 px-4 py-2.5 text-[10px] dark:border-blue-900/70 dark:bg-blue-950/20">
