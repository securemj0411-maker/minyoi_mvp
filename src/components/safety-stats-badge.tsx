...
function StatRow({ icon, label, count }: Row) {
  ...
  return (
    <div className="flex items-center justify-between gap-2">
      <span className="flex items-center gap-1.5 text-blue-800 dark:text-blue-300/90">
        <span className="text-blue-600 dark:text-blue-400">{icon}</span>
        <span>{label}</span>
      </span>
      <span className="font-mono font-bold tabular-nums text-blue-700 dark:text-blue-300">
...
function GroupHeader({ icon, label }: { icon: ReactNode; label: string }) {
  return (
    <div className="mt-2 flex items-center gap-1.5 border-t border-blue-200 pt-2 dark:border-blue-900">
...
export default function SafetyStatsBadge() {
  ...
  if (!stats) {
    return (
      <div className="mb-4 rounded-2xl border-2 border-blue-300 bg-blue-50 px-4 py-3 dark:border-blue-800 dark:bg-blue-950/30">
        <div className="flex w-full items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.18em] text-blue-700/80 dark:text-blue-400/80">
              <ShieldIcon />
              <span>회원님 보호</span>
            </div>
            <div className="mt-1 flex items-baseline gap-2">
              <span className="inline-block h-7 w-20 animate-pulse rounded bg-blue-200/80 dark:bg-blue-900/60" />
              <span className="text-sm font-bold text-blue-800/70 dark:text-blue-300/70">이번 주 차단된 매물</span>
            </div>
          </div>
          <span className="flex items-center gap-1 rounded-full bg-blue-600/40 px-3 py-1 text-[10px] font-black text-white/70">상세<ChevronDownIcon open={false} /></span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="relative mb-4 rounded-2xl border-2 border-blue-300 bg-blue-50 px-4 py-3 dark:border-blue-800 dark:bg-blue-950/30">
      <button
        type="button"
        onClick={hideForWeek}
        className="absolute right-2 top-2 inline-flex items-center gap-1 rounded-full bg-white/70 px-2 py-1 text-[10px] font-bold text-blue-800 transition hover:bg-white dark:bg-zinc-900/70 dark:text-blue-200"
      ...
      >
        <div>
          <div className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-[0.18em] text-blue-700 dark:text-blue-400">
            <ShieldIcon />
            <span>회원님 보호</span>
          </div>
          <div className="mt-1 flex items-baseline gap-2">
            ...
            <span className="text-sm font-bold text-blue-800 dark:text-blue-300/80">
              오늘 차단된 매물
            </span>
          </div>
        </div>
        <span className="flex items-center gap-1 rounded-full bg-blue-600 px-3 py-1 text-[10px] font-black text-white">
          {showDetail ? "접기" : "상세"}
          <ChevronDownIcon open={showDetail} />
        </span>
      </button>
      {showDetail && (
        <div className="mt-3 grid gap-1.5 border-t border-blue-200 pt-3 text-[11px] dark:border-blue-900">
          ...
          <div className="mt-2 text-[10px] leading-[1.5] text-blue-700/70 dark:text-blue-400/70">
