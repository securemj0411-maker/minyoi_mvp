  ...
  return (
    <section className="font-mono">
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <h2 className="text-xs font-black uppercase tracking-[0.18em] text-amber-400">▌FEEDBACK REVIEW (7D)</h2>
        <div className="ml-auto flex gap-1">
          {(["pending", "all", "approved", "rejected"] as StatusFilter[]).map((s) => (
            <button
              ...
              className={`rounded-sm border px-2 py-1 text-xs font-black uppercase tracking-wide transition ${
                statusFilter === s
          ...
          <button
            ...
            className="rounded-sm border border-zinc-800 bg-zinc-900 px-2 py-1 text-xs font-bold uppercase tracking-wide text-zinc-400 hover:border-zinc-700 hover:text-zinc-200"
          >REFRESH</button>
        </div>
      </div>

      {notice ? (
        <Notice tone="blue" className="mb-2">
          {notice}
        </Notice>
      ) : null}
      {error ? (
        <Notice tone="rose" className="mb-2">
          {error}
        </Notice>
      ) : null}
      ...
      <div className="space-y-2 md:hidden">
        {loading && rows.length === 0 ? (
          <div className="rounded-sm border border-zinc-800 bg-zinc-950 px-3 py-6 text-center text-xs uppercase text-zinc-400">불러오는 중…</div>
        ) : filtered.length === 0 ? (
          <div className="rounded-sm border border-zinc-800 bg-zinc-950 px-3 py-6 text-center text-xs uppercase text-zinc-400">결과 없음</div>
        ) : (
          filtered.map((r) => {
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-baseline gap-2">
                    ...
                    {r.category === "sold_out" && isPending ? (
                      <span className="rounded-sm border border-rose-700/60 bg-rose-900/40 px-1 py-0.5 text-xs font-black uppercase text-rose-300">⚠ 풀 제외</span>
                    ) : null}
                  </div>
                  <span className={`rounded-sm border px-1.5 py-0.5 text-xs font-black uppercase ${STATUS_BADGE[r.status] ?? STATUS_BADGE.pending}`}>{r.status}</span>
                </div>
                <div className="mt-1.5 text-xs text-zinc-300">
                  <div className="font-bold text-zinc-100">{r.user_nickname || "(닉네임 없음)"}</div>
                  <div className="font-mono text-xs text-zinc-500">{r.user_email ?? r.user_ref.slice(0, 24) + "…"}</div>
                </div>
                {r.pid ? (
                  <div className="mt-1 text-xs text-zinc-500">매물 #{r.pid}</div>
                ) : null}
                <div className="mt-2 max-h-24 overflow-y-auto rounded-sm border border-zinc-800 bg-zinc-900/40 px-2 py-1.5 text-xs text-zinc-200 whitespace-pre-wrap">
                  {r.message}
                </div>
                <div className="mt-1.5 font-mono text-xs text-zinc-500">{fmt(r.created_at)}</div>
                {isPending ? (
                  ...
                  <div className="mt-1 text-xs uppercase text-zinc-400">처리됨 · {r.decided_by ?? "—"}{r.reward_granted_at ? ` · +${r.reward_amount} @ ${fmt(r.reward_granted_at)}` : ""}</div>
                )}
              </div>
            );
          })
      ...
      <div className="hidden overflow-x-auto rounded-sm border border-zinc-800 bg-zinc-950 md:block">
        <table className="w-full min-w-[1100px] text-xs">
          <thead className="bg-zinc-900/80">
            <tr className="border-b border-zinc-800 text-left text-xs font-black uppercase tracking-[0.14em] text-zinc-500">
          ...
          <tbody>
            {loading && rows.length === 0 ? (
              <tr><td colSpan={8} className="px-3 py-6 text-center text-xs uppercase text-zinc-400">불러오는 중…</td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={8} className="px-3 py-6 text-center text-xs uppercase text-zinc-400">결과 없음</td></tr>
            ) : (
              filtered.map((r) => {
                const busy = pendingIds.has(r.id);
                ...
                return (
                  <Fragment key={r.id}>
                    <tr
                      ...
                      className={`cursor-pointer border-b border-zinc-900 outline-none transition hover:bg-zinc-900/40 focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-blue-400/70 ${
                    ...
                    >
                      ...
                      <td className="px-3 py-2 text-xs">
                        ...
                        <div className="font-mono text-xs text-zinc-500">{r.user_email ?? r.user_ref.slice(0, 18) + "…"}</div>
                      </td>
                      <td className="px-3 py-2 font-mono text-xs text-zinc-400">{r.pid ?? "—"}</td>
                      <td className="px-3 py-2 font-bold text-amber-300">
                        {CATEGORY_LABEL[r.category] ?? r.category}
                        {r.category === "sold_out" && isPending ? (
                          <span className="ml-1.5 rounded-sm border border-rose-700/60 bg-rose-900/40 px-1.5 py-0.5 text-xs font-black uppercase tracking-wide text-rose-300">
                            ⚠ 풀 제외 중
                          </span>
                        ) : null}
                      </td>
                      ...
                      <td className="px-3 py-2">
                        <span className={`rounded-sm border px-1.5 py-0.5 text-xs font-black uppercase ${STATUS_BADGE[r.status] ?? STATUS_BADGE.pending}`}>
                          {r.status}
                        </span>
                      </td>
                      <td className="px-3 py-2 font-mono text-xs text-zinc-500">{fmt(r.created_at)}</td>
                      <td className="px-3 py-2">
                        {isPending ? (
                          <div className="flex justify-end gap-1" onClick={(e) => e.stopPropagation()}>
                            <button
                              ...
                              onClick={() => void decide(r.id, "approve")}
                              disabled={busy}
                              className="rounded-sm border border-blue-800 bg-blue-900/40 px-2 py-1 text-xs font-black uppercase text-blue-300 hover:bg-blue-900/60 disabled:opacity-40"
                            >{busy ? "..." : "APPROVE +20"}</button>
                            <button
                              type="button"
                              onClick={() => void decide(r.id, "reject")}
                              disabled={busy}
                              className="rounded-sm border border-rose-800 bg-rose-900/40 px-2 py-1 text-xs font-black uppercase text-rose-300 hover:bg-rose-900/60 disabled:opacity-40"
                            >{busy ? "..." : "REJECT"}</button>
                          </div>
                        ) : (
                          <span className="text-xs uppercase text-zinc-400">{r.decided_by ?? "—"}</span>
                        )}
                      </td>
                    </tr>
                    {isExpanded ? (
                      <tr className="border-b border-zinc-900 bg-zinc-950/60">
                        <td colSpan={8} className="px-4 py-3">
                          <dl className="grid grid-cols-[120px_minmax(0,1fr)] gap-y-1.5 text-xs">
                            ...
                            <dd className="font-mono text-xs text-zinc-300">{r.user_email ?? "(없음)"}</dd>
                            <dt className="font-bold uppercase tracking-wide text-zinc-500">USER REF</dt>
                            <dd className="break-all font-mono text-xs text-zinc-400">{r.user_ref}</dd>
                            <dt className="font-bold uppercase tracking-wide text-zinc-500">AUTH ID</dt>
                            <dd className="break-all font-mono text-xs text-zinc-500">{r.auth_user_id}</dd>
                            {itemName ? (<>
                            ...
                            {r.decided_at ? (<>
                              ...
                              <dd className="font-mono text-xs text-zinc-400">{fmt(r.decided_at)} · by {r.decided_by ?? "—"}</dd>
                            </>) : null}
                            {r.reward_granted_at ? (<>
                              <dt className="font-bold uppercase tracking-wide text-zinc-500">REWARD</dt>
                              <dd className="font-mono text-xs text-blue-400">+{r.reward_amount} @ {fmt(r.reward_granted_at)}</dd>
                            </>) : null}
                          </dl>
                        </td>
                      </tr>
