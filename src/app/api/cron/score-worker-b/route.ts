...
function isDaangnBProject(): boolean {
  return String(process.env.CRON_PROJECT_ROLE ?? "").trim().toLowerCase() === "daangn_b";
}

function scoreWorkerBBudgetMs() {
  return envInt(["PIPELINE_SCORE_WORKER_B_BUDGET_MS", "PIPELINE_SCORE_WORKER_BUDGET_MS"], 55_000, 5_000, 55_000);
}

function scoreStageOptions(): ScoreStageOptions {
  const shardCount = envInt(["PIPELINE_SCORE_B_DAANGN_SHARD_COUNT", "PIPELINE_SCORE_DAANGN_SHARD_COUNT", "DAANGN_INGEST_B_REGION_SHARD_COUNT", "DAANGN_INGEST_REGION_SHARD_COUNT"], 2, 1, 20);
...
async function handleScoreWorkerB(req: NextRequest) {
  const { authOk, authReason } = checkCronAuth(req);
  const meta = buildCronRequestMeta(req, authOk, authReason, "score-worker-b");
  ...
  if (!envBool("PIPELINE_SCORE_WORKER_B_ENABLED", true)) {
    return NextResponse.json({ ok: true, started: false, skipped: true, mode: "score_worker_b", reason: "disabled" });
  }

  const guard = await acquireCronGuardWithSourceHealth("score_worker_b", req);
  if (!guard.allowed) {
    return NextResponse.json(cronGuardSkipBody(guard));
  }

  const config = loadPipelineRuntimeConfig();
  const budgetMs = scoreWorkerBBudgetMs();
  ...
  if (!run.id) {
    guard.release();
    return NextResponse.json(
      { ok: false, mode: "score_worker_b", error: "supabase_unavailable_before_pipeline", ts: run.startedAt },
      { status: 503 },
    );
  }
