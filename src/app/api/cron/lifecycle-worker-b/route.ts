// Wave 979 (2026-05-31): lifecycle-worker-b — daangn shard 1/3 전용.
//   score-worker-b 패턴 동일. base lifecycle-worker 가 bunjang/joongna 다 + daangn shard 0/3 처리,
//   b 가 daangn shard 1/3, c 가 daangn shard 2/3. capacity 9,600/h → 28,800/h (3 lane).
//
//   배경: lifecycle worker 백로그 12만+ 누적, daangn 363k active backfill 시 단일 worker 부족.
//   source/shard 분산으로 sold detection + velocity 정확성 보장.
...
function lifecycleBClaimOptions(): LifecycleClaimOptions {
  const shardCount = envInt(["PIPELINE_LIFECYCLE_B_DAANGN_SHARD_COUNT", "PIPELINE_LIFECYCLE_DAANGN_SHARD_COUNT"], 3, 1, 20);
  const shardIndex = envInt(["PIPELINE_LIFECYCLE_B_DAANGN_SHARD_INDEX"], 1, 0, Math.max(0, shardCount - 1));
  return {
    sourceFilter: "daangn",
...
function requestMeta(req: NextRequest, authOk: boolean, authReason: string): CollectRunRequestMeta {
  ...
  return {
    triggerSource: (headers.get("x-qstash-schedule-id") ?? userAgent ?? "lifecycle-worker-b").slice(0, 120),
...
async function executeLifecycleBRun(
  ...
  const claimOptions = lifecycleBClaimOptions();
  const run = await startCollectRun({
    ...meta,
    requestMeta: {
      ...meta.requestMeta,
      pipelineMode: guardMode,
      lifecycleMode: "daangn_shard_b",
      claimOptions: {
...
async function handleLifecycleWorkerB(req: NextRequest) {
  const { authOk, authReason } = checkCronAuth(req);
  const meta = requestMeta(req, authOk, authReason);
  const guardMode: CronWorkerMode = "lifecycle_worker_b";
  ...
  const outcome = await executeLifecycleBRun(meta, guard, guardMode);
  return NextResponse.json(outcome.body, { status: outcome.status });
}

export async function GET(req: NextRequest) {
  return handleLifecycleWorkerB(req);
}
