...
import { runPoolWarmerPipeline } from "@/lib/tick-pipeline";
import type { PipelineResult } from "@/lib/pipeline";

// Wave 724 (2026-05-23): p95 29s/max 85s 측정 → 120s buffer.
...
function requestMeta(req: NextRequest, authOk: boolean, authReason: string): CollectRunRequestMeta {
  ...
  return {
    triggerSource: (headers.get("x-qstash-schedule-id") ?? userAgent ?? "pool-warmer").slice(0, 120),
...
function toPipelineResult(result: Awaited<ReturnType<typeof runPoolWarmerPipeline>>): PipelineResult {
  return {
...
async function handlePoolWarmer(req: NextRequest) {
  ...
  const guard = await acquireCronGuardWithSourceHealth("pool_warmer", req);
  if (!guard.allowed) {
  ...
  if (!run.id) {
    ...
    return NextResponse.json(
      { ok: false, mode: "pool_warmer", error: "supabase_unavailable_before_pipeline", ts: run.startedAt },
      { status: 503 },
    );
  }

  try {
    const result = await runPoolWarmerPipeline();
