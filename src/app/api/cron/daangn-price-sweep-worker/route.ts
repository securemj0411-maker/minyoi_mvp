...
async function handleDaangnPriceSweepWorker(req: NextRequest) {
  ...
  try {
    const result = await runDaangnPriceSweep({
      ...
      maxSkus: envInt("DAANGN_PRICE_SWEEP_MAX_SKUS", 80, 1, 500),
      maxRegions: envInt("DAANGN_PRICE_SWEEP_MAX_REGIONS", 24, 1, 300),
      maxSearchCombos: envInt("DAANGN_PRICE_SWEEP_MAX_SEARCH_COMBOS", 120, 0, 2000),
      maxCategoryCombos: envInt("DAANGN_PRICE_SWEEP_MAX_CATEGORY_COMBOS", 180, 0, 2000),
      maxDetailFetches: envInt("DAANGN_PRICE_SWEEP_MAX_DETAIL_FETCHES", 140, 0, 1000),
      searchConcurrency: envInt("DAANGN_PRICE_SWEEP_SEARCH_CONCURRENCY", 36, 1, 200),
      detailConcurrency: envInt("DAANGN_PRICE_SWEEP_DETAIL_CONCURRENCY", 10, 1, 50),
      requestDelayMs: envInt("DAANGN_PRICE_SWEEP_REQUEST_DELAY_MS", 0, 0, 30_000),
      abortOnBlockedCombo: envBool("DAANGN_PRICE_SWEEP_ABORT_ON_BLOCKED_COMBO", false),
