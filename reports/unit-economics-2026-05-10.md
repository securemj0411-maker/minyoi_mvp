...
- 생성 시각: 2026. 5. 10. 오후 8:22:52
- 분석 구간: 최근 24시간 (2026-05-09T11:22:52.386Z 이후)
- 데이터 소스: Supabase 운영 로그, 후보팩 기록, AI 분류 비용 기록

## 한 줄 결론

주의 신호 2개가 있습니다: cron 실패율 63.8%로 높음.

## 운영 처리량

| 항목 | 최근 구간 | 일 환산 | 월 환산/참고 |
| --- | --- | --- | --- |
| cron/worker 실행 | 1,000 | 1,000 | 실패율 63.8% |
| QStash 메시지 | 917 | 917 | free 1,000/day 기준 |
| 검색 API 호출 | 1,791 | 1,791 | 번개장터 rate-limit 예산 |
| 수집된 검색 row | 260,770 | 260,770 | 중복 포함 observation |
| detail claim | 6,686 | 6,686 | 성공 6,539 / 실패 69 |
| score 계산 | 49,421 | 49,421 | pool upsert 14,973 |
| 함수 실행 시간 | 23359.8초 | 23359.8초/day | p95 35.8초 |

## Worker별 병목

| worker | runs | fail | avg sec | search calls | queued | detail | scored | pool | AI |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| tick | 744 | 630 | 24.4 | 1,508 | 14,663 | 66/80 | 34,200 | 1,715 | 314 (901 cache) |
| detail-worker | 148 | 0 | 29.9 | 0 | 0 | 5,374/5,455 | 0 | 0 | 0 (0 cache) |
| deep-crawl | 31 | 3 | 13.3 | 283 | 9,420 | 0/0 | 0 | 0 | 0 (0 cache) |
| housekeeper | 27 | 0 | 0.3 | 0 | 0 | 0/0 | 0 | 0 | 0 (0 cache) |
...
| Vercel 함수 비용 | 금액 미산정 | 23359.8초/day | 700793.9초/month | 플랜/메모리별 과금이라 시간 예산으로 추적 |
| Supabase 비용 | 금액 미산정 | 8,477 주요 API/day | DB row/read/write 별도 관찰 | 현재는 저장소/쿼리 병목 지표로 관리 |
...
| --- | --- |
| ready pool | 57건 |
| ready 평균 예상 순익 | 63,617원 |
| detail queue | done 987, pending 8, failed 5 |
| market key queue | pending 286, done 181 |
| market invalidation event | 1,298 |
