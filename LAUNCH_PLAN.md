...
- **base/default 옵션 가정 OK** (Wave 182 Phase 3, 2026-05-17 사용자 제안):
  - SKU 별 base option (가장 낮은 RAM/SSD/storage/size) 가정 + UI "기본 옵션 가정" 라벨 표시.
  - **§12b 안전성**: base 시세 = 가장 낮은 옵션 → priceGap underestimate → 추천 보수적 → **false positive 발생 X**.
  - recall loss 만 발생 (진짜 고옵션 매물이 base 시세로 비교돼 안 추천) — OK.
  - 적용 SKU: `src/lib/sku-base-options.ts` 의 `SKU_BASE_OPTIONS` 매핑 (Apple/Samsung 공식 spec).
  - 단 자급제 변형 (-256-self 등) / 단일 옵션 모델 / broad SKU 는 baseOptions 박지 X.
  - **이전 정책 (Wave 106 #54 — base 추정 금지)** 폐기. 이유: 정확도 risk 0 (base 시세 underestimate → 보수적). UI 라벨로 정직 표시.
...
| 2026-05-16 | 10-iteration batch review (사용자 지시). 6 자율 fix commit + push. | (1) cron-watchdog lookback 동적 (24h worker false positive 차단), (2) 4 cron route collect-logs 박기 (watchdog blind spot 해소), (3) tick-pipeline placeholder price ≥1억 시세/풀 차단 (14건 noise), (4) option-parser new_or_open_box false positive 차단 (~1368건 추정), (5) market/history + market-source rate limit (enumeration 차단), (6) MarketHistoryChart placeholder/429 UX. test 139/139 pass. | 사용자 결정 보류 7건 정리 (docs/SCRATCH/2026-05-15-batch-review-pending.md) |
| 2026-05-16 | Bunjang Detail API rate limit probe 실행 (600 calls / 6 phase). | **시나리오 A**: 모든 phase 200 응답, 429 0건. c=20에서 throughput 329 req/s까지 lenient. c=30에서 server soft throttle (latency 27x, 거절 X). 산출물: docs/EXPERIMENTS/2026-05-16-bunjang-rate-limit-probe-results.md + scripts/probe-bunjang-rate-limit-results.json | lifecycle worker 5x throughput 적용 |
| 2026-05-16 | lifecycle worker throughput 5x: batch 80→400 + sequential→Promise.all wave (concurrency 10). | 686 → 3,429 calls/h. backlog 2,659건 → 45분 안 해소 예상. maxDuration 90s 한도의 7% 사용. test 139/139 pass. commit be998ca. | 24h 측정 후 batch 800 c=15 (10x) step up 검토 |
| 2026-05-16 | cron watchdog 정책 정리: 베타 traffic 작은 2 cron 추적 제거 (landing-showcases + housekeeper-ai-cache-prune). compliance-retention + reference-price-refresh 유지. | 텔레그램 false positive alert 차단. 법적 책무 (compliance-retention) + 시세 정확도 (reference-price-refresh)만 추적. | 사용자 액션: compliance-retention QStash 등록 (1일 1회, 법적) |
